...
export function LandingPageClient({ bannerUrl }: { bannerUrl?: string | null }) {
  ...
  return (
    <AppPublicCheckInGlassPage className={cn("!px-0 !pt-0 sm:!px-0", appMobileDockContentClearanceClass)}>
      ...
      >
        <div className={cn("mx-auto flex max-w-6xl items-center justify-between gap-3 px-4 pb-3 sm:px-6 sm:pb-4", appSafeAreaLandingHeaderPadClass)}>
          <Link
            href="/"
        ...
        >
          ...
          <ul className="mt-6 grid gap-4 sm:grid-cols-3 sm:gap-6">
            {[
              ...
              {
                t: "หนึ่งแพลตฟอร์ม",
                d: "ทรัพย์สิน บันทึกส่วนตัว หอพัก หมู่บ้าน โรงเรียน ร้านค้า บริการ และเอกสาร — ไม่ต้องย้ายข้อมูลหลายที่",
              },
            ].map((x) => (
              <li
                key={x.t}
