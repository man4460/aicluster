        ...
        {ADMIN_HUB_NAV_ITEMS.map((item) => (
          <li key={item.href} className="min-w-0">
            <Link
              href={item.href}
              className={cn(
                "group flex min-h-[5.5rem] items-stretch justify-between gap-3 rounded-[1.25rem] border border-white/55 p-4 text-left shadow-[0_16px_40px_-28px_rgba(30,27,75,0.35)] transition-all duration-300 sm:min-h-[6rem] sm:rounded-[2rem] sm:p-5",
            ...
            >
              <span className="flex min-w-0 flex-1 items-start gap-3">
                ...
                <span className="min-w-0 flex-1 pt-0.5">
                  <span className="block break-words font-black leading-snug text-[#1e1b4b] sm:text-[15px]">
                    {item.label}
                  </span>
                  <span className="mt-1 block break-words text-xs leading-snug text-[#5f5a8a]">
                    {item.description}
                  </span>
                </span>
              </span>
