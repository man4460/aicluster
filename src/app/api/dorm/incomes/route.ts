...
function mapIncome(row: {
  ...
  return {
    ...
    categoryKind: row.category.kind as "TENANT_RENT" | "CUSTOM",
  };
}
...
export async function GET(req: Request) {
  ...
  try {
    ...
    const rows = await prisma.dormitoryIncomeEntry.findMany({
      where,
    ...
    console.error("dorm/incomes GET", e);
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const cat = await prisma.dormitoryIncomeCategory.findFirst({
      where: { id: categoryId, ownerUserId: auth.session.sub, trialSessionId: scope.trialSessionId },
    });
    if (!cat) return NextResponse.json({ error: "ไม่พบหมวดหมู่" }, { status: 400 });
    if (cat.kind !== "CUSTOM" || cat.isBuiltin) {
      return NextResponse.json(
        { error: "บันทึกรายรับเพิ่มได้เฉพาะหมวดที่สร้างเอง — ค่าเช่ามาจากการชำระ" },
        { status: 400 },
      );
    }

    const row = await prisma.dormitoryIncomeEntry.create({
      data: {
        ownerUserId: auth.session.sub,
        trialSessionId: scope.trialSessionId,
        categoryId,
    ...
    console.error("dorm/incomes POST", e);
    const msg = prismaErrorToApiMessage(e);
