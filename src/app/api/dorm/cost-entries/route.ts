...
export async function GET() {
  ...
  try {
    ...
    const rows = await prisma.dormitoryCostEntry.findMany({
    ...
    console.error("dorm/cost-entries GET", e);
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const row = await prisma.dormitoryCostEntry.create({
      data: {
        ownerUserId: auth.session.sub,
        trialSessionId: scope.trialSessionId,
        categoryId: cat.id,
    ...
    console.error("dorm/cost-entries POST", e);
    const msg = prismaErrorToApiMessage(e);
