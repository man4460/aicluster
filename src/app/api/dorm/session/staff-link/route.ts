...
export async function GET(req: Request) {
  try {
    ...
    if (!ctx || ctx.isStaff) return NextResponse.json({ error: "เฉพาะเจ้าของหอพัก" }, { status: 403 });
    const ownerId = ctx.billingUserId;
    let row = await prisma.dormitoryStaffLink.findUnique({
      where: {
    ...
    console.error("[dorm/session/staff-link GET]", e);
...
export async function POST(req: Request) {
  try {
    ...
    try {
      ...
      console.error("[dorm/session/staff-link POST] cipher", err);
      return NextResponse.json(
        { error: "ตั้ง AUTH_SECRET ใน .env ให้ยาวอย่างน้อย 16 ตัวอักษร — ใช้เข้ารหัสลิงก์พนักงาน" },
        { status: 500 },
      );
    }
    await prisma.dormitoryStaffLink.upsert({
      where: {
        ownerUserId_trialSessionId: {
          ownerUserId: ownerId,
          trialSessionId: STAFF_LINK_PERMANENT_SESSION_ID,
    ...
    console.error("[dorm/session/staff-link POST]", e);
