...
export async function GET() {
  ...
  try {
    ...
    const rows = await prisma.dormitoryCostCategory.findMany({
    ...
    console.error("dorm/cost-categories GET", e);
...
export async function POST(req: Request) {
  ...
  try {
    const scope = await getDormitoryDataScope(auth.session.sub);
    const row = await prisma.dormitoryCostCategory.create({
      data: {
    ...
    console.error("dorm/cost-categories POST", e);
    const msg = prismaErrorToApiMessage(e);
