...
export async function PATCH(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const existing = await prisma.dormitoryCostCategory.findFirst({
  ...
  try {
    const row = await prisma.dormitoryCostCategory.update({
      where: { id },
    ...
    console.error("dorm/cost-categories PATCH", e);
...
export async function DELETE(_req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    await prisma.dormitoryCostCategory.delete({ where: { id } });
    return NextResponse.json({ ok: true });
  } catch (e) {
    console.error("dorm/cost-categories DELETE", e);
    const msg = prismaErrorToApiMessage(e);
