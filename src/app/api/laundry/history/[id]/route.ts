...
const patchSchema = z.object({
  ...
  receiptImageUrl: z.union([laundryReceiptUrl, z.null()]).optional(),
...
export async function PATCH(req: Request, ctx: Ctx) {
  ...
  const log = await prisma.laundryServiceLog.findFirst({
  ...
  const logData: Prisma.LaundryServiceLogUncheckedUpdateInput = {};
  ...
  if (d.paymentMethod !== undefined && isCash) logData.paymentMethod = d.paymentMethod?.trim() || null;
  ...
  try {
    await prisma.$transaction(async (tx) => {
      if (Object.keys(custData).length > 0) {
        await tx.laundryCustomer.update({
          where: { id: log.laundryCustomerId },
          data: custData,
        });
      }
      if (Object.keys(logData).length > 0) {
        await tx.laundryServiceLog.update({
          where: { id },
          data: logData,
        });
      }
...
export async function DELETE(_req: Request, ctx: Ctx) {
  ...
  await prisma.$transaction(async (tx) => {
    if (row.visitType === "PACKAGE_USE" && row.subscriptionId != null) {
      ...
      if (sub) {
        ...
        await tx.laundryCustomerSubscription.update({
          ...
          data: {
            ...
            status: next > 0 && sub.status === "EXHAUSTED" ? "ACTIVE" : sub.status,
          },
        });
      }
    }
    await tx.laundryServiceLog.delete({ where: { id } });
  });
  ...
  await writeSystemActivityLog({
    ...
    action: "DELETE",
    modelName: "LaundryServiceLog",
    payload: { id, ownerUserId: own.ownerId, visitType: row.visitType },
  });
  return NextResponse.json({ ok: true });
}
