...
import { appSlipPaperSizeZod, normalizeModuleSlipPaperSize } from "@/lib/profile/module-slip-paper-size";
...
import { laundryMinutesToHm, laundryParseHmToMinutes } from "@/systems/laundry/lib/shop-hours";
import {
...
const hmTimeSchema = z
  ...
  .trim()
  .regex(/^\d{1,2}:\d{2}$/)
  .refine((v) => laundryParseHmToMinutes(v) != null, { message: "invalid HH:mm" });
...
const patchSchema = z
  .object({
    ...
    pickupFeePerKmBaht: z.number().int().min(0).max(9_999_999).nullable().optional(),
...
function profileFromRow(row: {
  ...
  const open = row.openTime && laundryParseHmToMinutes(row.openTime) != null ? row.openTime : "09:00";
  const close = row.closeTime && laundryParseHmToMinutes(row.closeTime) != null ? row.closeTime : "20:00";
  return {
...
const emptyProfile = {
  ...
  pickupFeePerKmBaht: null,
  promptPayQrImageUrl: null,
};

export async function GET() {
  const own = await laundryOwner();
  if (!own.ok) return own.res;
  const row = await prisma.laundryShopProfile.findUnique({
    where: {
...
export async function PATCH(req: Request) {
  ...
  const presetsSerialized =
    d.payAmountPresets !== undefined
      ? serializeLaundryPayAmountPresets(parseLaundryPayAmountPresets(d.payAmountPresets ?? ""))
      : undefined;
  const gallerySerialized =
    d.portalGallery !== undefined ? laundrySerializePortalGallery(d.portalGallery) : undefined;

  const openTime =
    d.openTime !== undefined
      ? (() => {
          const m = laundryParseHmToMinutes(d.openTime);
          return m == null ? undefined : laundryMinutesToHm(m);
        })()
      : undefined;
  const closeTime =
    d.closeTime !== undefined
      ? (() => {
          const m = laundryParseHmToMinutes(d.closeTime);
          return m == null ? undefined : laundryMinutesToHm(m);
        })()
      : undefined;

  let portalMode: LaundryPortalBookingPaymentMode | undefined;
  let depositAmountBaht: number | null | undefined;
  if (d.portalBookingPaymentMode !== undefined) {
    portalMode = normalizeLaundryPortalPaymentMode(d.portalBookingPaymentMode);
  ...
  if (openTime !== undefined && closeTime !== undefined) {
    const o = laundryParseHmToMinutes(openTime);
    const c = laundryParseHmToMinutes(closeTime);
  ...
  const portalPatch = {
    ...
    ...(d.shopLat !== undefined ? { shopLat: d.shopLat } : {}),
    ...(d.shopLng !== undefined ? { shopLng: d.shopLng } : {}),
    ...(d.pickupFeePerKmBaht !== undefined ? { pickupFeePerKmBaht: d.pickupFeePerKmBaht } : {}),
  };
  ...
  if (portalMode === "DEPOSIT" && depositAmountBaht === undefined) {
    const existing = await prisma.laundryShopProfile.findUnique({
      where: {
        ownerUserId_trialSessionId: { ownerUserId: own.userId, trialSessionId: own.trialSessionId },
      },
  ...
  const updated = await prisma.laundryShopProfile.upsert({
    where: {
      ownerUserId_trialSessionId: { ownerUserId: own.userId, trialSessionId: own.trialSessionId },
    },
    create: {
      ...
      address: d.address,
      ...(d.slipPaperSize !== undefined ? { slipPaperSize: normalizeModuleSlipPaperSize(d.slipPaperSize) } : {}),
  ...
  const pinHash = await loadLaundryStaffDailyPinHash(own.userId, own.trialSessionId);
