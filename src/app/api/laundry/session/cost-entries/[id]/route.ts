...
export async function PATCH(req: Request, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    const existing = await prisma.laundryCostEntry.findFirst({
    ...
    const row = await prisma.laundryCostEntry.update({
      where: { id },
      data: {
        categoryId,
        spentAt,
    ...
    return jsonLaundrySessionError(e, "laundry/session/cost-entries/[id] PATCH");
  }
}
...
export async function DELETE(_req: Request, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    await prisma.laundryCostEntry.delete({ where: { id } });
    return NextResponse.json({ ok: true });
  } catch (e) {
    return jsonLaundrySessionError(e, "laundry/session/cost-entries/[id] DELETE");
  }
}
