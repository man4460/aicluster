...
export async function GET() {
  try {
    ...
    const rows = await prisma.laundryCostEntry.findMany({
    ...
    if (isLaundryCostTableMissingP2021(e)) {
      console.warn(
        "[laundry/session/cost-entries GET] ตาราง laundry_cost_entries ยังไม่มี — รัน npx prisma migrate deploy แล้ว prisma generate",
      );
      return NextResponse.json(
        {
    ...
    return jsonLaundrySessionError(e, "laundry/session/cost-entries GET");
  }
}
...
export async function POST(req: Request) {
  try {
    ...
    const cat = await prisma.laundryCostCategory.findFirst({
      where: {
    ...
    const row = await prisma.laundryCostEntry.create({
      data: {
        ownerUserId: own.ownerId,
        trialSessionId: scope.trialSessionId,
        categoryId: cat.id,
    ...
    return jsonLaundrySessionError(e, "laundry/session/cost-entries POST");
  }
}
