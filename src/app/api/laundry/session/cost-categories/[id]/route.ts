...
export async function PATCH(req: Request, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    const existing = await prisma.laundryCostCategory.findFirst({
    ...
    const row = await prisma.laundryCostCategory.update({
      where: { id },
    ...
    return jsonLaundrySessionError(e, "laundry/session/cost-categories/[id] PATCH");
  }
}
...
export async function DELETE(_req: Request, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    await prisma.laundryCostCategory.delete({ where: { id } });
    return NextResponse.json({ ok: true });
  } catch (e) {
    return jsonLaundrySessionError(e, "laundry/session/cost-categories/[id] DELETE");
  }
}
