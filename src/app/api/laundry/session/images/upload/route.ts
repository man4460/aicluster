...
export async function POST(req: Request) {
  try {
    ...
    const dir = path.join(process.cwd(), "public", "uploads", "laundry");
    await mkdir(dir, { recursive: true });
    const filename = `${own.ownerId}-${Date.now()}.${ext}`;
    await writeFile(path.join(dir, filename), buf);
    return NextResponse.json({ imageUrl: `/uploads/laundry/${filename}` });
  } catch (e) {
    console.error("[laundry/session/images/upload]", e);
    const msg =
      e instanceof Error && /ENOENT|EACCES|EPERM|ENOTDIR/i.test(e.message) ?
        "บันทึกไฟล์ไม่สำเร็จ — ตรวจสิทธิ์โฟลเดอร์ public/uploads/laundry หรือรัน next dev จากโฟลเดอร์โปรเจกต์"
      : "อัปโหลดไม่สำเร็จ";
