...
export async function GET() {
  try {
    ...
    const rows = await prisma.laundryRevenueCategory.findMany({
    ...
    if (isLaundryRevenueTableMissingP2021(e)) {
      console.warn(
        "[laundry/session/revenue-categories GET] ตาราง laundry_revenue_categories ยังไม่มี — รัน npx prisma migrate deploy แล้ว prisma generate",
      );
      return NextResponse.json(
    ...
    return jsonLaundrySessionError(e, "laundry/session/revenue-categories GET");
  }
}
...
export async function POST(req: Request) {
  try {
    ...
    const row = await prisma.laundryRevenueCategory.create({
      data: {
    ...
    return jsonLaundrySessionError(e, "laundry/session/revenue-categories POST");
  }
}
