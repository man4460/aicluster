...
import { barberPortalSignaturePathPrefix } from "@/lib/barber/portal-signature";
...
export async function POST(req: Request) {
  ...
  const rl = rateLimit(`barber-portal-sig:${ip}`, 40, 60 * 60 * 1000);
  if (!rl.ok) {
  ...
  const dir = path.join(process.cwd(), "public", "uploads", "barber-portal-signatures");
  await mkdir(dir, { recursive: true });
  await writeFile(path.join(dir, filename), buf);

  return NextResponse.json({ imageUrl: `${barberPortalSignaturePathPrefix()}${filename}` });
}
