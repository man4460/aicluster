...
export async function POST(req: Request) {
  ...
  const rl = rateLimit(`barber-portal-ppqr:${ip}`, 40, 10 * 60 * 1000);
  if (!rl.ok) {
  ...
  const open = await isBarberCustomerPortalOpenForOwner(parsed.data.ownerId);
  ...
  const profile = await prisma.barberShopProfile.findUnique({
    where: {
