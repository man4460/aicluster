...
export async function GET() {
  ...
  try {
    ...
    const rows = await prisma.barberCostCategory.findMany({
    ...
    console.error("barber/cost-categories GET", e);
...
export async function POST(req: Request) {
  ...
  try {
    const scope = await getBarberDataScope(own.ownerId);
    const row = await prisma.barberCostCategory.create({
      data: {
    ...
    console.error("barber/cost-categories POST", e);
    const msg = prismaErrorToApiMessage(e);
    return NextResponse.json(
