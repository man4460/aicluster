...
export async function PATCH(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const existing = await prisma.barberCostCategory.findFirst({
  ...
  try {
    const row = await prisma.barberCostCategory.update({
      where: { id },
    ...
    console.error("barber/cost-categories PATCH", e);
...
export async function DELETE(_req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    await prisma.barberCostCategory.delete({ where: { id } });
    return NextResponse.json({ ok: true });
  } catch (e) {
    console.error("barber/cost-categories DELETE", e);
    const msg = prismaErrorToApiMessage(e);
    return NextResponse.json(
