...
export async function GET() {
  ...
  try {
    ...
    const rows = await prisma.barberCostEntry.findMany({
    ...
    console.error("barber/cost-entries GET", e);
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const row = await prisma.barberCostEntry.create({
      data: {
        ownerUserId: own.ownerId,
        trialSessionId: scope.trialSessionId,
        categoryId: cat.id,
    ...
    console.error("barber/cost-entries POST", e);
    const msg = prismaErrorToApiMessage(e);
    return NextResponse.json(
