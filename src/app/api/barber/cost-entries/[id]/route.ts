...
export async function PATCH(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const existing = await prisma.barberCostEntry.findFirst({
  ...
  if (parsed.data.category_id != null) {
    const cat = await prisma.barberCostCategory.findFirst({
      where: {
  ...
  try {
    const row = await prisma.barberCostEntry.update({
      where: { id },
      data: {
        categoryId,
        spentAt,
    ...
    console.error("barber/cost-entries PATCH", e);
...
export async function DELETE(_req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    await prisma.barberCostEntry.delete({ where: { id } });
    return NextResponse.json({ ok: true });
  } catch (e) {
    console.error("barber/cost-entries DELETE", e);
    const msg = prismaErrorToApiMessage(e);
    return NextResponse.json(
