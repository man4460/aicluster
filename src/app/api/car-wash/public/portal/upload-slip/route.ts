...
function ownerTag(ownerId: string): string {
  return ownerId.replace(/[^a-zA-Z0-9_-]/g, "").slice(0, 24) || "owner";
}

export async function POST(req: Request) {
  const ip = clientIp(req.headers);
  const rl = rateLimit(`car-wash-portal-upload:${ip}`, 30, 60 * 60 * 1000);
  if (!rl.ok) {
  ...
  const dir = path.join(process.cwd(), "public", "uploads", "car-wash-portal-slips");
  await mkdir(dir, { recursive: true });
  await writeFile(path.join(dir, filename), buf);

  const imageUrl = `/uploads/car-wash-portal-slips/${filename}`;
