...
export async function PATCH(req: Request, ctx: RouteCtx) {
  ...
  try {
    ...
    const existing = await prisma.villageIncomeCategory.findFirst({
    ...
    const row = await prisma.villageIncomeCategory.update({
      where: { id },
      data: {
        ...(name ? { name } : {}),
    ...
    console.error("village/income-categories PATCH", e);
...
export async function DELETE(_req: Request, ctx: RouteCtx) {
  ...
  try {
    ...
    await prisma.villageIncomeCategory.delete({ where: { id } });
    return NextResponse.json({ ok: true });
  } catch (e) {
    console.error("village/income-categories DELETE", e);
    const msg = prismaErrorToApiMessage(e);
