...
export async function PATCH(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const existing = await prisma.villageCostEntry.findFirst({
  ...
  try {
    const row = await prisma.villageCostEntry.update({
      where: { id },
      data: {
        categoryId,
        spentAt,
    ...
    console.error("village/cost-entries PATCH", e);
...
export async function DELETE(_req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    await prisma.villageCostEntry.delete({ where: { id } });
    return NextResponse.json({ ok: true });
  } catch (e) {
    console.error("village/cost-entries DELETE", e);
    const msg = prismaErrorToApiMessage(e);
