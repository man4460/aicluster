...
export async function POST(req: Request) {
  try {
    ...
    const filename = `${ctx.ownerId}-${Date.now()}.${ext}`;
    await writeFile(path.join(dir, filename), buf);
    return NextResponse.json({ imageUrl: `/uploads/building-pos/${filename}` });
  } catch (e) {
    console.error("[building-pos/staff/images/upload]", e);
    const msg =
      e instanceof Error && /ENOENT|EACCES|EPERM|ENOTDIR/i.test(e.message) ?
        "บันทึกไฟล์ไม่สำเร็จ — ตรวจสิทธิ์โฟลเดอร์ public/uploads/building-pos"
