...
export async function POST(req: Request) {
  ...
  const rl = rateLimit(`bpos-portal-upload:${ip}`, 30, 60 * 60 * 1000);
  ...
  const portalOk = await isBuildingPosPortalOpenForOwner(ownerId);
  ...
  const dir = path.join(process.cwd(), "public", "uploads", "building-pos-portal", ownerSeg);
  const filename = `slip-${Date.now()}-${Math.random().toString(36).slice(2, 10)}.${ext}`;
  await mkdir(dir, { recursive: true });
  await writeFile(path.join(dir, filename), buf);

  return NextResponse.json({
    imageUrl: `/uploads/building-pos-portal/${ownerSeg}/${filename}`,
  });
}
