...
export async function POST(req: Request) {
  ...
  const dir = path.join(process.cwd(), "public", "uploads", "general-store-pos", ownerSeg);
  ...
  try {
    ...
    console.error("[general-store-pos/upload]", e);
    const msg =
      e instanceof Error && /ENOENT|EACCES|EPERM|ENOTDIR/i.test(e.message) ?
        "บันทึกไฟล์ไม่สำเร็จ — ตรวจสิทธิ์โฟลเดอร์ public/uploads/general-store-pos หรือรันแอปจากโฟลเดอร์โปรเจกต์"
      : "บันทึกไฟล์ไม่สำเร็จ";
    return NextResponse.json({ error: msg }, { status: 500 });
  }

  const imageUrl = `/uploads/general-store-pos/${ownerSeg}/${filename}`;
