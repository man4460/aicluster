...
export async function PATCH(req: Request, ctx: RouteCtx) {
  ...
  const existing = await prisma.generalStorePosCategory.findFirst({
    where: { id, ownerUserId },
    select: { id: true },
  });
  if (!existing) return NextResponse.json({ error: "ไม่พบหมวด" }, { status: 404 });

  const row = await prisma.generalStorePosCategory.update({
    where: { id },
    data: {
...
export async function DELETE(_req: Request, ctx: RouteCtx) {
  ...
  await prisma.generalStorePosCategory.delete({ where: { id } });
