...
export async function PATCH(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const existing = await prisma.ecommerceCostCategory.findFirst({
  ...
  const row = await prisma.ecommerceCostCategory.update({
    where: { id },
    data: {
      ...(name ? { name } : {}),
...
export async function DELETE(_req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  await prisma.ecommerceCostCategory.delete({ where: { id } });
