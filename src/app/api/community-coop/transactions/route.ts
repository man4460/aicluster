...
import {
  assertCommunityCoopAccountOwned,
  getCommunityCoopOwnerContext,
} from "@/systems/community-coop/lib/community-coop-api-auth";
...
export async function POST(req: Request) {
  ...
  let type: CommunityCoopLedgerType;
  if (kind === "deposit" || kind === "dividend") {
    type = kind === "dividend" ? CommunityCoopLedgerType.DIVIDEND : CommunityCoopLedgerType.DEPOSIT;
  } else if (kind === "withdraw") type = CommunityCoopLedgerType.WITHDRAW;
  else return NextResponse.json({ error: "invalid_kind" }, { status: 400 });

  const owned = await assertCommunityCoopAccountOwned(accountId, ctx.userId, ctx.scope.trialSessionId);
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      const acc = await tx.communityCoopAccount.findUnique({ where: { id: accountId } });
      if (!acc) return { ok: false as const, code: "not_found" as const };
      const isAdd =
        type === CommunityCoopLedgerType.DEPOSIT || type === CommunityCoopLedgerType.DIVIDEND;
      const delta = isAdd ? amountSatang : -amountSatang;
