...
export async function GET() {
  ...
  const rows = await prisma.drinkPosCostCategory.findMany({
...
export async function POST(req: Request) {
  ...
  const maxSort = await prisma.drinkPosCostCategory.aggregate({
    where: { ownerUserId: auth.ctx.ownerUserId },
    _max: { sortOrder: true },
  });
  const sortOrder =
  ...
  const row = await prisma.drinkPosCostCategory.create({
