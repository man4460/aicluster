...
export async function DELETE(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const found = await prisma.assetSupplier.findFirst({
    where: { id: idNum, ownerUserId, trialSessionId },
    select: { id: true },
  });
  if (!found) return NextResponse.json({ error: "ไม่พบผู้ขาย" }, { status: 404 });
  await prisma.assetSupplier.update({ where: { id: idNum }, data: { isActive: false } });
