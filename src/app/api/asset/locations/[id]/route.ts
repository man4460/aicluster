...
export async function DELETE(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const found = await prisma.assetLocation.findFirst({
    where: { id: idNum, ownerUserId, trialSessionId },
    select: { id: true },
  });
  if (!found) return NextResponse.json({ error: "ไม่พบสถานที่" }, { status: 404 });
  await prisma.assetLocation.update({ where: { id: idNum }, data: { isActive: false } });
