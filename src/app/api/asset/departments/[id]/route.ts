...
export async function DELETE(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const found = await prisma.assetDepartment.findFirst({
    where: { id: idNum, ownerUserId, trialSessionId },
    select: { id: true },
  });
  if (!found) return NextResponse.json({ error: "ไม่พบแผนก" }, { status: 404 });
  await prisma.assetDepartment.update({ where: { id: idNum }, data: { isActive: false } });
