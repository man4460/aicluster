...
const UpsertSchema = z.object({
  ...
  headPersonName: z.string().trim().max(120).nullable().optional(),
...
export async function GET() {
  ...
  const items = await prisma.assetDepartment.findMany({
...
export async function POST(req: Request) {
  ...
  let item;
  if (data.id) {
    item = await prisma.assetDepartment.update({
      where: { id: data.id },
      data: {
        code: data.code,
        name: data.name,
        headPersonName: data.headPersonName ?? null,
    ...
    item = await prisma.assetDepartment.create({
      data: {
        ownerUserId,
        trialSessionId,
        code: data.code,
