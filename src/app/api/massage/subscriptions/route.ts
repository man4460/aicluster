...
import {
  normalizeMassageSlipUrlForDashboard,
  parseMassageCashReceiptBasenameFromStored,
} from "@/lib/massage/receipt-display-url";
 ...
 * ห้ามใช้ isPrismaSchemaMismatch (P2022 ทุกคอลัมน์) หรือ isPrismaSchemaMismatchError ที่แมตช์ `massage_customer_subscriptions`
...
export async function GET(req: Request) {
  ...
  const mapRow = (
    ...
    saleReceiptImageUrl: (() => {
      ...
      if (!raw) return null;
      if (parseMassageCashReceiptBasenameFromStored(raw)) {
        return `/api/massage/subscriptions/${s.id}/sale-receipt`;
      }
      const norm = normalizeMassageSlipUrlForDashboard(raw, requestOrigin);
      if (norm && parseMassageCashReceiptBasenameFromStored(norm)) {
        return `/api/massage/subscriptions/${s.id}/sale-receipt`;
      }
      return norm;
    })(),
    ...
    soldByTherapist: s.soldByTherapist
      ? { id: s.soldByTherapist.id, name: s.soldByTherapist.name }
      : null,
  });
  ...
  const tries: Array<() => Promise<Parameters<typeof mapRow>[0][]>> = [
    async () => {
      const rows = await prisma.massageCustomerSubscription.findMany({
        where: whereSub,
        orderBy,
        take,
        select: {
    ...
    async () => {
      ...
      return rows.map((r) => ({ ...r, soldByTherapist: null }));
    },
    async () => {
      const rows = await prisma.massageCustomerSubscription.findMany({
        where: whereSub,
  ...
  for (const run of tries) {
    try {
      ...
      if (!isPrismaSchemaMismatch(e) && !isPrismaSchemaMismatchError(e)) {
        ...
        console.error("[massage/subscriptions GET]", e);
        return NextResponse.json(
  ...
  console.error("[massage/subscriptions GET] fallbacks exhausted", lastErr);
...
export async function POST(req: Request) {
  ...
  try {
    ...
    let soldByTherapistId: number | null = null;
    if (parsed.data.therapistId != null) {
      const st = await prisma.massageTherapist.findFirst({
        where: {
          id: parsed.data.therapistId,
    ...
    const baseData = {
      ...
      packageId: pkg.id,
      remainingSessions,
      ...(soldByTherapistId != null ? { soldByTherapistId } : {}),
    };

    let sub: { id: number; remainingSessions: number; status: string };
    let receiptSkipped = false;
    try {
      sub = await prisma.massageCustomerSubscription.create({
        data: {
          ...baseData,
      ...
      if (retry) {
        ...
        receiptSkipped = true;
        try {
          await prisma.massageCustomerSubscription.update({
            where: { id: sub.id },
            data: { saleReceiptImageUrl: receiptUrl },
          });
          receiptSkipped = false;
        } catch (ue) {
          console.warn("[massage/subscriptions POST] บันทึกสลิปหลังสร้างแพ็กไม่สำเร็จ:", ue);
        }
      } else {
        throw e;
      }
    ...
    if (receiptUrl) {
      try {
        const saved = await prisma.massageCustomerSubscription.findUnique({
          where: { id: sub.id },
        ...
        console.warn("[massage/subscriptions POST] backfill saleReceiptImageUrl:", bf);
        receiptSkipped = true;
      }
    }
    ...
    const saleReceiptImageUrlForClient = receiptUrl?.trim()
      ? `/api/massage/subscriptions/${sub.id}/sale-receipt`
      : null;
    ...
    console.error("[massage/subscriptions POST]", e);
    ...
    return NextResponse.json(
      {
        error: `บันทึกไม่สำเร็จ — ${PRISMA_SYNC_HINT_TH} หรือตรวจสอบ MySQL / DATABASE_URL (ดู log: [massage/subscriptions POST])`,
      },
      { status: 500 },
    );
  }
