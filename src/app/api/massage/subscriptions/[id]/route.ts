...
const massageSaleReceiptUrl = z
  .string()
  .max(512)
  .regex(/^\/uploads\/massage-cash-receipts\/[a-zA-Z0-9._-]+$/i);
...
const patchSchema = z.object({
  ...
  saleReceiptImageUrl: z.union([massageSaleReceiptUrl, z.null()]).optional(),
});
...
export async function GET(_req: Request, ctx: Ctx) {
  ...
  const sub = await prisma.massageCustomerSubscription.findFirst({
...
export async function PATCH(req: Request, ctx: Ctx) {
  ...
  if (parsed.data.customerName !== undefined) {
    await prisma.massageCustomer.update({
      where: { id: sub.massageCustomerId },
      data: { name: parsed.data.customerName?.trim() || null },
    });
  }
  const next = await prisma.massageCustomerSubscription.update({
    where: { id },
    data: {
  ...
  await writeSystemActivityLog({
    ...
    action: "UPDATE",
    modelName: "MassageCustomerSubscription",
...
export async function DELETE(_req: Request, ctx: Ctx) {
  ...
  try {
    await prisma.$transaction(async (tx) => {
      await tx.massageServiceLog.updateMany({
        where: { subscriptionId: id },
        data: { subscriptionId: null },
      });
      await tx.massagePortalStaffPing.updateMany({
        where: { subscriptionId: id },
        data: { subscriptionId: null },
      });
      await tx.massageCustomerSubscription.delete({ where: { id } });
    });
  } catch (e) {
    console.error("[massage/subscriptions DELETE]", id, e);
