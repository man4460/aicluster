...
import { parseMassageCashReceiptBasenameFromStored } from "@/lib/massage/receipt-display-url";
...
export async function GET(_req: Request, ctx: Ctx) {
  ...
  let sub = await prisma.massageCustomerSubscription.findFirst({
    where: { id, ownerUserId: own.ownerId, trialSessionId: scope.trialSessionId },
    select: { saleReceiptImageUrl: true },
  });
  if (!sub) {
    sub = await prisma.massageCustomerSubscription.findFirst({
  ...
  const basename = parseMassageCashReceiptBasenameFromStored(sub.saleReceiptImageUrl);
  ...
  const dir = path.join(process.cwd(), "public", "uploads", "massage-cash-receipts");
