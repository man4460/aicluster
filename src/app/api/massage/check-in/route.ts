...
const cashSchema = z.object({
  ...
  therapistId: z.number().int().positive().optional().nullable(),
  receiptImageUrl: massageCashReceiptUrl.optional().nullable(),
});
...
export async function POST(req: Request) {
  ...
  async function resolveTherapistId(therapistId: number | null | undefined): Promise<number | null> {
    if (therapistId == null) return null;
    const st = await prisma.massageTherapist.findFirst({
      where: { id: therapistId, ownerUserId: ownerId, trialSessionId, isActive: true },
    });
    if (!st) return null;
    return st.id;
  }
  ...
  if ("subscriptionId" in parsed.data) {
    ...
    const therapistIdResolved = await resolveTherapistId(parsed.data.therapistId);
  ...
  try {
    ...
    try {
      await prisma.massageServiceLog.create({
        data: {
          ...cashLogCore,
      ...
      if (staleClientReceipt) {
        console.warn(
          "[massage/check-in] Prisma ไม่รู้จัก receiptImageUrl (มักเป็น client ค้างหลัง prisma generate) — บันทึกโดยไม่แนบรูปสลิป",
        );
        await prisma.massageServiceLog.create({ data: cashLogCore });
      } else {
        throw e;
      }
    }
    ...
    console.error("[massage/check-in] CASH_WALK_IN", e);
