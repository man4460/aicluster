...
export async function GET(req: Request) {
  ...
  const row = await prisma.massageBooking.findFirst({
    where: {
      id: bookingId,
      ownerUserId: ownerId,
      trialSessionId,
  ...
  const [branding, profile] = await Promise.all([
    getQrMassageBranding(ownerId, trialSessionId),
  ...
  return NextResponse.json({
    shop: {
      displayName: profile?.displayName?.trim() || branding.label || "ร้านนวด",
    ...
    booking: {
      ...
      therapistName: row.therapist?.name?.trim() || null,
