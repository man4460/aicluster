...
export async function POST(req: Request) {
  ...
  const rl = rateLimit(`massage-portal-upload:${ip}`, 30, 60 * 60 * 1000);
  if (!rl.ok) {
  ...
  const dir = path.join(process.cwd(), "public", "uploads", "massage-portal-slips");
  await mkdir(dir, { recursive: true });
  await writeFile(path.join(dir, filename), buf);

  const imageUrl = `${massagePortalSlipPathPrefix()}${filename}`;
