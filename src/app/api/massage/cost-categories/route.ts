...
export async function GET() {
  ...
  try {
    ...
    const rows = await prisma.massageCostCategory.findMany({
    ...
    console.error("massage/cost-categories GET", e);
    const msg = prismaErrorToApiMessage(e);
    return NextResponse.json(
      { error: msg ?? "โหลดหมวดต้นทุนไม่สำเร็จ" },
      { status: 500 },
    );
  }
}
...
export async function POST(req: Request) {
  ...
  try {
    const scope = await getMassageDataScope(own.ownerId);
    const row = await prisma.massageCostCategory.create({
      data: {
    ...
    console.error("massage/cost-categories POST", e);
    const msg = prismaErrorToApiMessage(e);
    return NextResponse.json(
      { error: msg ?? "สร้างหมวดต้นทุนไม่สำเร็จ" },
      { status: 500 },
    );
  }
}
