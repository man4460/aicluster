...
export async function GET(req: Request) {
  ...
  try {
    const rows = await prisma.massageTherapist.findMany({
      where: {
    ...
    console.error("[massage/therapists GET]", e);
...
export async function POST(req: Request) {
  ...
  const s = await prisma.massageTherapist.create({
    data: {
      ownerUserId: own.ownerId,
      trialSessionId: scope.trialSessionId,
