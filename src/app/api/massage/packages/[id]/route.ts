...
export async function PATCH(req: Request, ctx: Ctx) {
  ...
  const existing = await prisma.massagePackage.findFirst({
  ...
  if (d.totalSessions !== undefined) {
    const hasSubs = await prisma.massageCustomerSubscription.count({
  ...
  const p = await prisma.massagePackage.update({
    where: { id },
    data: {
...
export async function DELETE(_req: Request, ctx: Ctx) {
  ...
  try {
    await prisma.massagePackage.delete({ where: { id } });
  } catch {
    return NextResponse.json(
