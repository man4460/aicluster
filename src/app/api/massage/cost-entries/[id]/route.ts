...
export async function PATCH(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const existing = await prisma.massageCostEntry.findFirst({
  ...
  if (parsed.data.category_id != null) {
    const cat = await prisma.massageCostCategory.findFirst({
      where: {
  ...
  try {
    const row = await prisma.massageCostEntry.update({
      where: { id },
      data: {
        categoryId,
        spentAt,
    ...
    console.error("massage/cost-entries PATCH", e);
    const msg = prismaErrorToApiMessage(e);
    return NextResponse.json(
      { error: msg ?? "แก้ไขรายจ่ายไม่สำเร็จ" },
      { status: 500 },
    );
  }
}
...
export async function DELETE(_req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    await prisma.massageCostEntry.delete({ where: { id } });
    return NextResponse.json({ ok: true });
  } catch (e) {
    console.error("massage/cost-entries DELETE", e);
    const msg = prismaErrorToApiMessage(e);
    return NextResponse.json(
      { error: msg ?? "ลบรายจ่ายไม่สำเร็จ" },
      { status: 500 },
    );
  }
}
