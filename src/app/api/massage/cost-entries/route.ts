...
export async function GET() {
  ...
  try {
    ...
    const rows = await prisma.massageCostEntry.findMany({
    ...
    console.error("massage/cost-entries GET", e);
    const msg = prismaErrorToApiMessage(e);
    return NextResponse.json(
      { error: msg ?? "โหลดรายการต้นทุนไม่สำเร็จ" },
      { status: 500 },
    );
  }
}
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const row = await prisma.massageCostEntry.create({
      data: {
        ownerUserId: own.ownerId,
        trialSessionId: scope.trialSessionId,
        categoryId: cat.id,
    ...
    console.error("massage/cost-entries POST", e);
    const msg = prismaErrorToApiMessage(e);
    return NextResponse.json(
      { error: msg ?? "บันทึกรายจ่ายไม่สำเร็จ" },
      { status: 500 },
    );
  }
}
