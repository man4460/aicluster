...
export async function PATCH(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const existing = await prisma.hotelResortCostCategory.findFirst({
  ...
  const row = await prisma.hotelResortCostCategory.update({
    where: { id },
    data: {
      ...(name ? { name } : {}),
...
export async function DELETE(_req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  await prisma.hotelResortCostCategory.delete({ where: { id } });
