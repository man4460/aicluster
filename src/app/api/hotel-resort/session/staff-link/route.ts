...
export async function GET(req: Request) {
  try {
    ...
    let row = await prisma.hotelResortStaffLink.findUnique({
      where: {
    ...
    console.error("[hotel-resort/session/staff-link GET]", e);
...
export async function POST(req: Request) {
  try {
    ...
    try {
      ...
      console.error("[hotel-resort/session/staff-link POST] cipher", err);
      return NextResponse.json(
        { error: "ตั้ง AUTH_SECRET ใน .env ให้ยาวอย่างน้อย 16 ตัวอักษร — ใช้เข้ารหัสลิงก์พนักงาน" },
        { status: 500 },
      );
    }
    await prisma.hotelResortStaffLink.upsert({
      where: {
        ownerUserId_trialSessionId: {
          ownerUserId: ownerId,
          trialSessionId: STAFF_LINK_PERMANENT_SESSION_ID,
    ...
    console.error("[hotel-resort/session/staff-link POST]", e);
