...
export async function POST(req: Request) {
  try {
    ...
    const dir = path.join(process.cwd(), "public", "uploads", "lms", own.ownerId);
    await mkdir(dir, { recursive: true });
    const filename = `${own.ownerId}-${Date.now()}.${ext}`;
    await writeFile(path.join(dir, filename), buf);
    return NextResponse.json({ imageUrl: `/uploads/lms/${own.ownerId}/${filename}` });
  } catch (e) {
    console.error("[lms/session/images/upload]", e);
