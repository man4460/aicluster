...
export async function PATCH(req: Request, ctx: Ctx) {
  try {
    ...
    const existing = await prisma.lmsFinanceTransaction.findFirst({
    ...
    const row = await prisma.lmsFinanceTransaction.update({
      where: { id },
      data: {
    ...
    console.error("[lms/session/finance/[id] PATCH]", e);
...
export async function DELETE(_req: Request, ctx: Ctx) {
  try {
    ...
    await prisma.lmsFinanceTransaction.delete({ where: { id } });
    return NextResponse.json({ ok: true });
  } catch (e) {
    console.error("[lms/session/finance/[id] DELETE]", e);
