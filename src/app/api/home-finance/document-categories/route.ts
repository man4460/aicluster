      ...
      res: NextResponse.json(
        {
          error:
            ctx?.isStaff === true
              ? "บัญชีพนักงานไม่สามารถใช้บันทึกส่วนตัวได้ — โปรดเข้าด้วยบัญชีเจ้าของ"
...
export async function GET() {
  ...
  try {
    const rows = await prisma.homeFinanceDocumentCategory.findMany({
      where: { ownerUserId: guard.billingUserId },
      orderBy: [{ sortOrder: "asc" }, { id: "asc" }],
    });
    return NextResponse.json({ categories: rows.map(mapCategory) });
  } catch (e) {
    console.error("home-finance/document-categories GET", e);
    return NextResponse.json({ error: "โหลดหมวดเอกสารไม่สำเร็จ" }, { status: 500 });
  }
}
...
export async function POST(req: Request) {
  ...
  try {
    const row = await prisma.homeFinanceDocumentCategory.create({
      data: {
