...
export async function PATCH(req: Request, ctx: Ctx) {
  ...
  const existing = await prisma.homeFinanceDocumentCategory.findFirst({
  ...
  try {
    const row = await prisma.homeFinanceDocumentCategory.update({
      where: { id },
      data: {
    ...
    // sync denormalized label on documents
...
export async function DELETE(_req: Request, ctx: Ctx) {
  ...
  const linked = await prisma.homeFinancePersonalDocument.count({
    where: { ownerUserId: guard.billingUserId, categoryId: id },
  });
  if (linked > 0) {
    return NextResponse.json(
      { error: `ยังมีเอกสาร ${linked} รายการในหมวดนี้ — ย้ายหรือลบเอกสารก่อน` },
      { status: 409 },
    );
  }

  await prisma.homeFinanceDocumentCategory.delete({ where: { id } });
