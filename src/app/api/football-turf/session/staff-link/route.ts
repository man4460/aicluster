...
export async function GET(req: Request) {
  try {
    ...
    let row = await prisma.footballTurfStaffLink.findUnique({
      where: {
      ...
      try {
        ...
        if (plain) {
          url = buildStaffPortalUrl({
            req,
            pathPrefix: "/football-turf/staff",
            ownerId,
            plainToken: plain,
          });
        }
    ...
    return NextResponse.json({ configured: true as const, url, module: FOOTBALL_TURF_MODULE_SLUG });
  } catch (e) {
    console.error("[football-turf/session/staff-link GET]", e);
...
export async function POST(req: Request) {
  try {
    ...
    try {
      ...
      console.error("[football-turf/session/staff-link POST] cipher", err);
      return NextResponse.json(
        { error: "ตั้ง AUTH_SECRET ใน .env ให้ยาวอย่างน้อย 16 ตัวอักษร — ใช้เข้ารหัสลิงก์พนักงาน" },
        { status: 500 },
      );
    }
    await prisma.footballTurfStaffLink.upsert({
      where: {
        ownerUserId_trialSessionId: {
          ownerUserId: ownerId,
          trialSessionId: STAFF_LINK_PERMANENT_SESSION_ID,
    ...
    console.error("[football-turf/session/staff-link POST]", e);
