/** ชำระตอนจองคาร์แคร์ — แม่แบบสนามฟุตบอล / โรงแรม */

export type CarWashPortalBookingPaymentMode = "NONE" | "DEPOSIT" | "FULL";

export function normalizeCarWashPortalPaymentMode(
  value: string | null | undefined,
): CarWashPortalBookingPaymentMode {
  if (value === "DEPOSIT" || value === "FULL" || value === "NONE") return value;
  return "NONE";
}

/** ยอดที่ต้องชำระตอนจอง — null = ไม่บังคับ */
export function carWashComputePortalPayDue(opts: {
  mode: CarWashPortalBookingPaymentMode;
...
export function carWashPortalSlipProofMessage(mode: CarWashPortalBookingPaymentMode): string {
...
export function carWashComputeBookingPaymentStatus(
  totalBaht: number,
