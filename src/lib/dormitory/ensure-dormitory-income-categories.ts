...
const BUILTIN_INCOME = [{ kind: "TENANT_RENT" as const, name: "ค่าเช่า / หอพัก", sortOrder: 0 }];

const DEFAULT_CUSTOM_INCOME = [{ name: "รายรับอื่น", sortOrder: 10 }];

/** สร้างหมวดรายรับหลัก (ค่าเช่า) + หมวด CUSTOM เริ่มต้น — และลบหมวดหลักที่ซ้ำจาก race */
export async function ensureDormitoryIncomeCategories(ownerUserId: string, trialSessionId: string) {
  for (const row of BUILTIN_INCOME) {
    const existing = await prisma.dormitoryIncomeCategory.findMany({
      where: {
        ownerUserId,
        trialSessionId,
        kind: row.kind,
    ...
    if (existing.length === 0) {
      try {
        await prisma.dormitoryIncomeCategory.create({
          data: {
            ownerUserId,
            trialSessionId,
            name: row.name,
    ...
    if (keep && keep.name !== row.name) {
      await prisma.dormitoryIncomeCategory.update({
        where: { id: keep.id },
        data: { name: row.name },
      });
    }
    if (dupes.length > 0) {
      await prisma.dormitoryIncomeCategory.deleteMany({
        where: { id: { in: dupes.map((d) => d.id) } },
      });
    }
  }

  const customs = await prisma.dormitoryIncomeCategory.findMany({
  ...
  for (const seedName of DEFAULT_CUSTOM_INCOME.map((r) => r.name)) {
    ...
    const withEntries = await prisma.dormitoryIncomeEntry.groupBy({
      by: ["categoryId"],
