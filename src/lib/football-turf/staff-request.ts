...
export type FootballTurfStaffContext = PermanentStaffLinkContext;

export async function resolveFootballTurfStaffFromUrl(url: URL): Promise<FootballTurfStaffContext | null> {
  ...
  return resolvePermanentStaffLink({
    ...
    urlTrialParam: t,
    findProdRow: () =>
      prisma.footballTurfStaffLink.findUnique({
        where: {
    ...
    findAnyRows: () =>
      prisma.footballTurfStaffLink.findMany({
        where: { ownerUserId: ownerId },
        select: { tokenHash: true },
      }),
    liveDataScope: () => getFootballTurfDataScope(ownerId),
  });
}
