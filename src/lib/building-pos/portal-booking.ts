/** ชำระตอนจองจากเว็บลูกค้า — POS ร้านอาหาร */

export type BuildingPosPortalBookingPaymentMode = "NONE" | "DEPOSIT" | "FULL";

export const BUILDING_POS_PORTAL_GALLERY_MAX = 8;
export const BUILDING_POS_REVIEW_PHOTO_MAX = 5;

export function normalizeBuildingPosPortalPaymentMode(
  value: string | null | undefined,
): BuildingPosPortalBookingPaymentMode {
 ...
 */
export function buildingPosComputePortalPayDue(opts: {
  mode: BuildingPosPortalBookingPaymentMode;
...
export function buildingPosPortalSlipProofMessage(mode: BuildingPosPortalBookingPaymentMode): string {
...
export function buildingPosNormalizePortalGallery(raw: unknown): string[] {
  let arr: unknown[] = [];
  ...
  for (const item of arr) {
    ...
    out.push(url);
    if (out.length >= BUILDING_POS_PORTAL_GALLERY_MAX) break;
  }
  return out;
}

export function buildingPosNormalizeReviewPhotos(raw: unknown): string[] {
  let arr: unknown[] = [];
  if (typeof raw === "string") {
    try {
      const parsed = JSON.parse(raw) as unknown;
  ...
  return arr
    ...
    .map((u) => u.trim())
    .slice(0, BUILDING_POS_REVIEW_PHOTO_MAX);
}

export type BuildingPosPortalCartItem = {
  menuItemId: number;
  name: string;
  unitPrice: number;
  qty: number;
...
export function buildingPosNormalizePortalCartItems(raw: unknown): BuildingPosPortalCartItem[] {
  if (!Array.isArray(raw)) return [];
  const out: BuildingPosPortalCartItem[] = [];
  for (const row of raw) {
    if (!row || typeof row !== "object") continue;
    const r = row as Record<string, unknown>;
    const menuItemId = Math.round(Number(r.menuItemId ?? r.id));
    ...
    if (!Number.isFinite(menuItemId) || menuItemId <= 0 || !name || qty <= 0) continue;
    out.push({ menuItemId, name, unitPrice, qty });
  }
  return out.slice(0, 40);
}

export function buildingPosCartItemsTotalBaht(items: BuildingPosPortalCartItem[]): number {
