...
async function deleteSandboxRowsInTx(tx: Tx, ownerUserId: string, trialSessionId: string): Promise<void> {
  ...
  await tx.laundryRevenueEntry.deleteMany({ where: { ownerUserId, trialSessionId } });
  await tx.laundryRevenueCategory.deleteMany({ where: { ownerUserId, trialSessionId } });
  ...
  await tx.laundryServiceLog.deleteMany({ where: { ownerUserId, trialSessionId } });
  await tx.laundryCustomerSubscription.deleteMany({ where: { ownerUserId, trialSessionId } });
  await tx.laundryOrder.deleteMany({ where: { ownerUserId, trialSessionId } });
  await tx.laundryCustomer.deleteMany({ where: { ownerUserId, trialSessionId } });
  await tx.laundryPackage.deleteMany({ where: { ownerUserId, trialSessionId } });
  await tx.laundryShopProfile.deleteMany({ where: { ownerUserId, trialSessionId } });
  await tx.clubEventGalleryImage.deleteMany({ where: { ownerUserId, trialSessionId } });
  await tx.clubEventRecord.deleteMany({ where: { ownerUserId, trialSessionId } });
  await tx.clubEventMember.deleteMany({ where: { ownerUserId, trialSessionId } });
  await tx.clubEventFinanceTransaction.deleteMany({ where: { ownerUserId, trialSessionId } });
  ...
  await tx.dormitoryIncomeEntry.deleteMany({ where: { ownerUserId, trialSessionId } });
  await tx.dormitoryIncomeCategory.deleteMany({ where: { ownerUserId, trialSessionId } });
