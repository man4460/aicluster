 ...
 * - ใช้พาธ `/uploads/massage-cash-receipts|massage-portal-slips/...` (เสิร์ฟที่ app/uploads/[bucket]/[filename])
 * - ถ้าเป็น absolute URL แต่ path อยู่ใต้ /uploads/ สองบัคเก็ตนี้ → ตัดเหลือพาธบน origin ของคำขอ (กันโดเมน/proxy คนละค่า)
 * - รองรับของเก่า `/api/massage/cash-receipt/file?name=...`
 */

const BARBER_SLIP_BUCKETS = ["massage-cash-receipts", "massage-portal-slips"] as const;
...
export function parseMassageCashReceiptBasenameFromStored(stored: string | null | undefined): string | null {
  if (!stored?.trim()) return null;
  const u = stored.trim();
  const patterns = [
    /\/uploads\/massage-cash-receipts\/([^/?#]+)/i,
    /^uploads\/massage-cash-receipts\/([^/?#]+)/i,
    /(?:^|[/"'])massage-cash-receipts\/([^/?#'"]+)/i,
  ];
...
export function normalizeMassageSlipUrlForDashboard(
  ...
  if (/\/api\/massage\/subscriptions\/\d+\/sale-receipt(?:\?.*)?$/i.test(pathForSaleReceiptTest)) {
  ...
  try {
    ...
    if (parsed.pathname.includes("/api/massage/cash-receipt/file")) {
      const name = parsed.searchParams.get("name")?.trim() ?? "";
      if (name && safeUploadBasename(name)) {
        return `/uploads/massage-cash-receipts/${name}`;
      }
    }
  } catch {
    /* ignore */
