  ...
  const ext = subtype.includes("png")
    ...
    : subtype.includes("webp")
      ...
      : subtype.includes("gif")
        ? "gif"
        : "jpg";
  const userSegment = resolveOwnerUploadSegment(userId);
...
`.trim();
  ...
  const intro = [
    ...
    "- บันทึกส่วนตัว: บันทึก 100 บาท / บันทึกรายจ่าย 500 บาท ค่ากาแฟ / ค่ายา 132 บาท (สั้นๆ) / บันทึกรายรับ … (ท้าย \"หมวด อาหาร\" หรือ \"#อาหาร\" ได้; ค่าเริ่มรายจ่ายถ้าไม่ระบุรายรับ/รายจ่าย)",
    "- รูปสลิปจากแชทเว็บ: ฝั่งเซิร์ฟเวอร์จะอ่านด้วย Kimi ก่อน แล้ว fallback เป็น GLM-OCR อัตโนมัติ; ถ้าอ่านยังไม่สำเร็จและเปิด TELEGRAM_SLIP_FALLBACK_ENABLED=1 จึงค่อยส่ง Telegram ทางสำรอง; ถ้าอ่านได้และมีจำนวนเงิน ระบบจะเตรียมรายการรอบันทึก — ชวนให้ผู้ใช้พิมพ์ **ยืนยัน** / **บันทึกเลย** / **ตกลง** เพื่อบันทึกลงบันทึกส่วนตัว (เมื่อมี `[ผลการเรียกเครื่องมือ]` ถึงถือว่าบันทึกสำเร็จ)",
...
function buildSlipAnalysisReply(args: {
  ...
  if (args.amountBaht != null && Number.isFinite(args.amountBaht) && args.amountBaht > 0) {
    lines.push("ถ้าข้อมูลถูกต้อง พิมพ์ **ยืนยัน** หรือ **บันทึกเลย** เพื่อบันทึกลงบันทึกส่วนตัวค่ะ");
  } else {
