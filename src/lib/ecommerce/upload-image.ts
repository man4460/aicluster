...
export async function saveEcommerceUploadImage(
  ...
  const dir = path.join(process.cwd(), "public", "uploads", "ecommerce-store", subdir, ownerSeg);
  ...
  try {
    ...
    console.error("[ecommerce/upload]", e);
    return { ok: false, error: "บันทึกไฟล์ไม่สำเร็จ", status: 500 };
  }

  return { ok: true, imageUrl: `/uploads/ecommerce-store/${subdir}/${ownerSeg}/${filename}` };
}
