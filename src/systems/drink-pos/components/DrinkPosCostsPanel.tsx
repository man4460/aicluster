...
export type DrinkPosCostCategoryRow = {
  id: string;
  name: string;
  sortOrder: number;
};

export type DrinkPosCostEntryRow = {
  id: string;
  label: string;
  amountBaht: number;
  spentAt: string;
...
async function uploadDrinkPosSlip(file: File): Promise<string> {
...
/** รายจ่าย — ต้นแบบจาก BuildingPosCostsPanel */
export function DrinkPosCostsPanel({
  categories,
  entries,
  onChanged,
  emptyWhenFilteredMessage,
}: {
  categories: DrinkPosCostCategoryRow[];
  entries: DrinkPosCostEntryRow[];
  ...
  const [catEditing, setCatEditing] = useState<DrinkPosCostCategoryRow | null>(null);
  const [catName, setCatName] = useState("");
  const [catBusy, setCatBusy] = useState(false);
  const [catErr, setCatErr] = useState<string | null>(null);

  const [costModalOpen, setCostModalOpen] = useState(false);
  const [costEditing, setCostEditing] = useState<DrinkPosCostEntryRow | null>(null);
  ...
  function openCatEdit(c: DrinkPosCostCategoryRow) {
    setCatEditing(c);
    setCatName(c.name);
    setCatErr(null);
    setCatFormOpen(true);
  ...
  async function submitCategory() {
    ...
    try {
      if (catEditing) {
        const res = await fetch(`/api/drink-pos/cost-categories/${catEditing.id}`, {
          method: "PATCH",
        ...
        const res = await fetch("/api/drink-pos/cost-categories", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
          body: JSON.stringify({ name }),
  ...
  async function deleteCategory(c: DrinkPosCostCategoryRow) {
    ...
    setCatErr(null);
    try {
      const res = await fetch(`/api/drink-pos/cost-categories/${c.id}`, {
        method: "DELETE",
        credentials: "include",
      });
      const j = await res.json().catch(() => ({}));
  ...
  function openCostEdit(entry: DrinkPosCostEntryRow) {
  ...
  async function uploadCostSlip(file: File) {
    ...
    setCostErr(null);
    try {
      setCostSlipUrl(await uploadDrinkPosSlip(file));
    } catch (err) {
  ...
  async function submitCost() {
    ...
    try {
      ...
      const res = await fetch(
        costEditing ? `/api/drink-pos/costs/${costEditing.id}` : "/api/drink-pos/costs",
        {
  ...
  async function deleteCost(entry: DrinkPosCostEntryRow) {
    const ok = await notice.confirm(`ลบรายจ่าย «${entry.label}» ใช่หรือไม่?`);
    if (!ok) return;
    try {
      const res = await fetch(`/api/drink-pos/costs/${entry.id}`, {
        method: "DELETE",
        credentials: "include",
      });
      const j = await res.json().catch(() => ({}));
