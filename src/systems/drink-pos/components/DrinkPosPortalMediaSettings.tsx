...
import { DRINK_POS_PORTAL_GALLERY_MAX } from "@/lib/drink-pos/portal-booking";
import {
...
export function DrinkPosPortalMediaSettings({
  bannerUrl,
  gallery,
  address,
  facebookUrl,
  ...
  async function uploadFile(file: File): Promise<string> {
    ...
    const res = await fetch("/api/drink-pos/upload", {
      method: "POST",
      body: fd,
      credentials: "include",
    });
  ...
  async function onPickGallery(e: ChangeEvent<HTMLInputElement>) {
    ...
    const slots = DRINK_POS_PORTAL_GALLERY_MAX - gallery.length;
    if (slots <= 0) {
      setErr(`อัปโหลดได้ไม่เกิน ${DRINK_POS_PORTAL_GALLERY_MAX} รูป`);
      return;
    }
    setUploadBusy(true);
    setErr(null);
    try {
      ...
      onGalleryChange([...gallery, ...added].slice(0, DRINK_POS_PORTAL_GALLERY_MAX));
    } catch (e2) {
    ...
    <div className="space-y-5 rounded-2xl border border-[#ecebff] bg-[#faf9ff]/80 p-3 sm:p-4">
      ...
      <div>
        <p className="text-xs font-bold text-[#4d47b6]">
          แกลเลอรี ({gallery.length}/{DRINK_POS_PORTAL_GALLERY_MAX})
        </p>
        <input
          ref={galleryPickRef}
          type="file"
          ...
          <AppImagePickCameraButtons
            ...
            onPickCamera={() =>
              galleryCamera.openCamera(async (file) => {
                if (gallery.length >= DRINK_POS_PORTAL_GALLERY_MAX) {
                  setErr(`อัปโหลดได้ไม่เกิน ${DRINK_POS_PORTAL_GALLERY_MAX} รูป`);
                  return;
                }
                setUploadBusy(true);
                ...
                try {
                  ...
                  onGalleryChange([...gallery, url].slice(0, DRINK_POS_PORTAL_GALLERY_MAX));
                } catch (e2) {
                  setErr(e2 instanceof Error ? e2.message : "อัปโหลดไม่สำเร็จ");
                } finally {
                  setUploadBusy(false);
            ...
            disabled={busy || gallery.length >= DRINK_POS_PORTAL_GALLERY_MAX}
            busy={uploadBusy}
