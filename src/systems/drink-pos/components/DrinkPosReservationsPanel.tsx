...
export function DrinkPosReservationsPanel({ embedded = false }: { embedded?: boolean }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      q.set("date", bangkokDateKey());
      const res = await fetch(`/api/drink-pos/session/reservations?${q}`, {
        credentials: "include",
        cache: "no-store",
      });
  ...
  const patch = async (body: Record<string, unknown>) => {
    ...
    setBusyId(id);
    try {
      const res = await fetch("/api/drink-pos/session/reservations", {
        method: "PATCH",
        credentials: "include",
      ...
      <div className="mt-3 flex flex-wrap gap-1.5" role="tablist" aria-label="กรองสถานะจอง">
        {STATUS_FILTERS.map((f) => (
          <button
            ...
            className={statusFilter === f.id ? drinkPosChipActiveClass : drinkPosChipIdleClass}
      ...
      {loading ? (
        ...
        <ul className="mt-4 space-y-3">
          {rows.map((r) => (
            <li
              key={r.id}
              className="flex flex-col gap-3 rounded-[1.25rem] border border-white/70 bg-white/70 p-3 sm:flex-row sm:items-center"
            >
              ...
              <div className="flex shrink-0 flex-wrap gap-2">
                {r.status === "SCHEDULED" ? (
                  <button
                    ...
                    onClick={() => void patch({ id: r.id, status: "ARRIVED" })}
                  >
                    มาแล้ว
                  </button>
                ) : null}
  ...
  if (embedded) {
    return <div className="space-y-1 rounded-[1.25rem] border border-white/60 bg-white/55 p-3 sm:p-4">{body}</div>;
  }
