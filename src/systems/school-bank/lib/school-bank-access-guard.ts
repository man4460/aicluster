...
export type SchoolBankAccessFailReason = "no_module" | "staff" | "not_subscribed" | "no_plan";

export async function loadSchoolBankAccessState(userId: string): Promise<
  | { ok: true; mod: { id: string; slug: string; groupId: number }; billingUserId: string }
  | { ok: false; reason: SchoolBankAccessFailReason }
> {
  const mod = await prisma.appModule.findFirst({
    where: { slug: SCHOOL_BANK_MODULE_SLUG, isActive: true },
  ...
  if (ctx.isStaff && !STAFF_ALLOWED_MODULE_SLUGS.has(SCHOOL_BANK_MODULE_SLUG)) {
