...
export function MassagePortalBookingClient({
  ownerId,
  bookingId,
  phone,
  trialSessionId,
  ...
  const homeHref = massagePublicPortalUrl("", ownerId, trialSessionId || "prod");
  ...
  useEffect(() => {
    ...
    setBusy(true);
    setErr(null);
    void fetch(`/api/massage/public/portal/booking?${q}`, { cache: "no-store" })
      .then(async (res) => {
        ...
        {busy ? (
          ...
          <>
            ...
            <section className={cn(appPublicCheckInGlassCardClass, "px-5 py-2 sm:px-6")}>
              ...
              {booking.therapistName ? <Row label="นักบำบัด" value={booking.therapistName} /> : null}
