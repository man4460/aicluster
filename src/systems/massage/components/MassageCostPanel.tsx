...
import {
  createMassageCostCategory,
  createMassageCostEntry,
  deleteMassageCostCategory,
  deleteMassageCostEntry,
  updateMassageCostCategory,
  updateMassageCostEntry,
  uploadMassageCostSlip,
  type MassageCostCategory,
  type MassageCostEntry,
} from "@/systems/massage/MassageCostClient";
import type { ModuleCostPanelOps } from "@/systems/massage/module-cost-panel-ops";

const defaultMassageCostPanelOps: ModuleCostPanelOps = {
  createCategory: createMassageCostCategory,
  updateCategory: updateMassageCostCategory,
  deleteCategory: deleteMassageCostCategory,
  createEntry: createMassageCostEntry,
  updateEntry: updateMassageCostEntry,
  deleteEntry: deleteMassageCostEntry,
  uploadSlip: uploadMassageCostSlip,
};

export type MassageCostToolbarApi = {
  ...
  return (
    <div className="rounded-[2rem] border border-violet-200/70 bg-gradient-to-br from-[#faf8ff] via-white to-rose-50/40 p-4 shadow-sm ring-1 ring-violet-100/60">
      ...
      {slipUrl.trim() && previewUrl ?
        <div className={`mt-4 flex flex-wrap items-center gap-3 ${massageCardSurfaceRadiusClass} border border-slate-200/80 bg-white/80 p-3`}>
...
export function MassageCostPanel({
  baseUrl,
  categories,
  entries,
  onRefresh,
  ...
  onToolbarReady,
  costPanelOps,
  formAriaIdPrefix = "massage-cost",
  renderWithoutOuterSection = false,
}: {
  baseUrl: string;
  categories: MassageCostCategory[];
  ...
  const ops = costPanelOps ?? defaultMassageCostPanelOps;
  ...
  const [editCat, setEditCat] = useState<MassageCostCategory | null>(null);
  ...
  const [editEntry, setEditEntry] = useState<MassageCostEntry | null>(null);
  ...
  const openEditCategoryForm = useCallback((c: MassageCostCategory) => {
    setEditCat(c);
    setCatName(c.name);
  }, []);
  ...
  useEffect(() => {
    ...
    const api: MassageCostToolbarApi = { openRecordExpense, openManageCategories };
    onToolbarReady(api);
  ...
  const openEditEntry = useCallback((e: MassageCostEntry) => {
    setEditEntry(e);
    setEditEntryForm({
  ...
  const categoryManagerSection = (
    <div className="space-y-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:gap-3">
        ...
        <div className="flex shrink-0 flex-wrap gap-2">
          {editCat ?
            <>
              <button
                ...
                className={`app-btn-primary ${massageCardSurfaceRadiusClass} px-4 py-2 text-sm font-semibold disabled:opacity-50`}
              >
                บันทึก
              </button>
              <button
                ...
                className={`${massageCardSurfaceRadiusClass} border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700 shadow-sm hover:bg-slate-50`}
              >
                ยกเลิก
              </button>
            </>
          : <button
              ...
              onClick={() => void submitCategory()}
              className={`app-btn-primary inline-flex items-center gap-2 ${massageCardSurfaceRadiusClass} px-4 py-2 text-sm font-semibold disabled:opacity-50`}
            >
      ...
      : <ul className={`max-h-[min(40vh,14rem)] divide-y divide-slate-100 overflow-y-auto ${massageCardSurfaceRadiusClass} border border-slate-200 bg-white`}>
          {categories.map((c) => (
  ...
  return (
    <div className="space-y-6">
      {fetchError ? (
        <p className={`${massageCardSurfaceRadiusClass} border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-950`}>
      ...
      {renderWithoutOuterSection ?
        <>
          {listLoading ?
            <p
              className={`${massageCardSurfaceRadiusClass} border border-dashed border-[#e8e6f4]/90 bg-gradient-to-br from-[#faf9ff]/90 via-white to-[#fff7ed]/30 ${massageCardBodyPaddingXClass} py-6 text-center text-sm text-slate-600`}
            >
              กำลังโหลดรายการ…
            </p>
        ...
        footer={
          <div className="flex justify-end">
            <button
              ...
              className={`${massageCardSurfaceRadiusClass} border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 shadow-sm hover:bg-slate-50`}
            >
              ปิด
            </button>
          </div>
