...
import {
  ...
  massageModalBackdropMutedClass,
  massageModalCloseBtnClass,
  massageModalHeaderClass,
  massageModalPanel2xlFlexColClass,
...
export function MassageUsageGuideModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  useEffect(() => {
    if (!open) return;
  ...
  return (
    <MassageModalPortal>
      <div className={massageModalBackdropMutedClass} role="presentation" onClick={onClose}>
        <div
          role="dialog"
          aria-modal="true"
          aria-labelledby="massage-usage-guide-title"
          className={massageModalPanel2xlFlexColClass}
          onClick={(e) => e.stopPropagation()}
        >
          <div className={massageModalHeaderClass}>
            <div className="min-w-0">
              <h2 id="massage-usage-guide-title" className={massageModalTitleClass}>
                คู่มือการใช้งาน — ร้านนวด
              </h2>
          ...
          <div className="space-y-5">
            ...
            <Section title="การเงิน">
              <p>
                ...
                URL ตรง:{" "}
                <code className="rounded-[1rem] bg-[#f0eeff] px-1 py-0.5 text-xs">/dashboard/massage/finance</code>
                {" · "}
                แท็บต้นทุน:{" "}
            ...
            <Section title="ตั้งค่า → ลิงก์ QR (ลูกค้า + พนักงาน)">
              ...
              <ul className="list-disc space-y-1.5 pl-5 marker:text-[#4d47b6]">
                <li>สร้าง QR ไปยังพอร์ทัลลูกค้า — ลูกค้าจองคิวตามช่วงเวลาร้าน หรือกรอกเบอร์เพื่อหักแพ็กเอง</li>
            ...
            <Section title="โปรไฟล์ร้าน (ร่วมกับทุกระบบ)">
              ...
              <ul className="list-disc space-y-1.5 pl-5 marker:text-[#4d47b6]">
                <li>โมดูลร้านนวดอ่านข้อมูลเดียวกับโปรไฟล์นี้ (ไม่มีหน้าตั้งค่าแยกในเมนูร้านนวด)</li>
            ...
            <Section title={APP_USAGE_GUIDE_MOBILE_CHROME_SECTION.title}>
        ...
        <div className="shrink-0 border-t border-slate-100 bg-white px-5 py-4">
          <button
            type="button"
            onClick={onClose}
            className={`app-btn-primary min-h-[48px] w-full ${massageCardSurfaceRadiusClass} px-4 py-3 text-sm font-semibold text-white sm:w-auto`}
          >
            ปิด
          </button>
        </div>
