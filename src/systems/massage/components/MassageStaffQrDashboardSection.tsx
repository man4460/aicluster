...
const BARBER_STAFF_QR_TAGLINE =
  "สแกนเข้าหน้าพนักงาน — แดชบอร์ดคิว/เช็กอิน และการจัดการ (ล็อกอินร้าน · ถ้าร้านตั้งรหัสรายวันต้องใส่รหัสก่อน)";
...
export function MassageStaffQrDashboardSection({
  ownerId,
  shopLabel,
  logoUrl,
  baseUrl,
  ...
  const staffPageUrl = useMemo(() => {
    ...
    if (!root) return "";
    const u = new URL("/dashboard/massage/staff", root);
  ...
  async function downloadStaffQrPng() {
    ...
    try {
      ...
      await downloadPosterPng(canvas, `massage-staff-qr-${ownerId.slice(0, 8)}.png`);
    } finally {
  ...
  async function downloadStaffQrPdf(format: "a4" | "a5") {
    ...
    try {
      ...
      await downloadPosterPdf(canvas, `massage-staff-qr-${format}-${ownerId.slice(0, 8)}.pdf`, format);
    } finally {
      setStaffQrBusy(false);
    }
  }
      ...
      <div className={massageQrHubPanelClass}>
        ...
        <div className={cn("space-y-3", compactForModal ? "mt-0" : "mt-6")}>
          ...
          <ShopStaffQrPanel
            ...
            posterAlt="ตัวอย่างโปสเตอร์ QR พนักงานร้านนวด"
          />
        </div>
      </div>
    </section>
