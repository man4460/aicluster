...
function MassageStaffClientInner({
  ...
  const runBootstrap = useCallback(async () => {
    ...
    const r = await fetch(`/api/massage/staff/bootstrap${qs}`, {
      cache: "no-store",
      credentials: "include",
      headers: staffDailyUnlockHeaders("massage", ownerId),
    });
    if (!r.ok) {
      setBootOk(false);
      setNeedsPin(false);
  ...
  const tabBtn = (active: boolean, compact?: boolean) =>
    cn(
      ...
      active
        ? cn(massageNavActiveClass, "ring-white/55")
        : cn("bg-white/50 ring-white/60", massageNavIdleClass),
    );
  ...
  if (needsPin) {
    return (
      <StaffDailyPinGate
        ...
        ownerId={ownerId}
        shopLabel={shopLabel}
        unlockApiPath="/api/massage/staff/unlock"
        staffQuery=""
        onUnlocked={() => {
  ...
  return (
    <>
      ...
      <div className="lg:hidden">
        <AppMobileDockUnifiedBar ariaLabel="เมนูพนักงานร้านนวด">
