...
import { normalizeTimeHHmm } from "@/lib/massage/slot-times";
import { massageCardBodyPaddingXClass } from "@/systems/massage/components/massage-ui-tokens";
...
function ScheduleFormSkeleton() {
  return (
    <div className={cn(massageCardBodyPaddingXClass, "space-y-4 pb-2")} aria-hidden>
...
export function MassageDayScheduleClient({
  ...
  const load = useCallback(async (d: string) => {
    ...
    setMsg(null);
    try {
      const res = await fetch(`/api/massage/day-schedules?date=${encodeURIComponent(d)}`, {
        credentials: "include",
      });
  ...
  async function save() {
    ...
    try {
      const res = await fetch("/api/massage/day-schedules", {
        method: "PUT",
        credentials: "include",
  ...
  const inner = !mounted ? (
    ...
    <div className={cn(massageCardBodyPaddingXClass, "space-y-4 pb-2")}>
