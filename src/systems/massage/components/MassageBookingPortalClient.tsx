...
import { MASSAGE_PORTAL_SAMPLE_BANNER } from "@/systems/massage/lib/portal-media";
import { massageFormatWorkWeekdaysLabel } from "@/systems/massage/lib/therapist-schedule";
import { massagePublicBookingUrl } from "@/lib/massage/public-url";
import {
  massageFindFirstFreeRun,
...
type PortalShop = {
  ...
  closeTime: string;
  slotMinutes: 30 | 60;
  portalBookingPaymentMode?: MassagePortalBookingPaymentMode;
...
export function MassageBookingPortalClient({
  ownerId,
  trialSessionId,
}: {
  ownerId: string;
  ...
  const [therapists, setTherapists] = useState<PortalTherapist[]>([]);
  ...
  const [therapistDayOff, setTherapistDayOff] = useState(false);
  ...
  const slotMinutes = shop ? massageNormalizeSlotMinutes(shop.slotMinutes) : 30;
  ...
  useEffect(() => {
    void fetch(`/api/massage/public/portal/info?${qBase}`, { cache: "no-store" })
      .then(async (res) => {
        if (!res.ok) {
        ...
        setShop({
          ...
          slotMinutes: massageNormalizeSlotMinutes(j.shop.slotMinutes ?? 30),
        });
        setPackages(
          (j.packages ?? []).map((p) => ({
            ...p,
            durationMinutes: massageNormalizeDurationMinutes(p.durationMinutes ?? 30, 30),
          })),
        );
        const st = j.therapists ?? [];
        setTherapists(st);
        if (st.length === 1) setTherapistId(st[0]!.id);
        setDurationMinutes(massageNormalizeSlotMinutes(j.shop.slotMinutes ?? 30));
  ...
  useEffect(() => {
    if (!shop) return;
    const needTherapist = therapists.length > 0;
    if (needTherapist && therapistId == null) {
      setAvailSlots([]);
      setSelectedSlots([]);
      setTherapistDayOff(false);
      return;
    }
    let cancelled = false;
    setAvailBusy(true);
    ...
    if (therapistId != null) q.set("therapistId", String(therapistId));
    void fetch(`/api/massage/public/portal/availability?${q}`, { cache: "no-store" })
      .then(async (res) => {
        const j = (await res.json().catch(() => ({}))) as {
          slots?: AvailSlot[];
          therapistDayOff?: boolean;
          error?: string;
        };
        if (cancelled) return;
        if (!res.ok) throw new Error(j.error ?? "โหลดสล็อตไม่สำเร็จ");
        setTherapistDayOff(Boolean(j.therapistDayOff));
  ...
  }, [shop, bookDate, therapistId, therapists.length, ownerId, resolvedTrial, trialSessionId]);
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const res = await fetch("/api/massage/public/portal/promptpay-qr", {
          method: "POST",
  ...
  function applyDurationAndAutoSelect(nextDuration: number) {
    const dur = massageNormalizeDurationMinutes(nextDuration, slotMinutes);
    setDurationMinutes(dur);
    const need = massageSlotsNeeded(dur, slotMinutes);
    const times = availSlots.map((s) => s.startTime);
    const busy: Array<{ startMin: number; endMin: number }> = [];
    for (const s of availSlots) {
      if (s.available) continue;
      const sm = massageParseHmToMinutes(s.startTime);
      if (sm == null) continue;
      busy.push({ startMin: sm, endMin: sm + slotMinutes });
    }
    const run = massageFindFirstFreeRun(times, busy, need, slotMinutes);
  ...
  async function lookupMember() {
    ...
    try {
      ...
      const res = await fetch("/api/massage/public/portal/member-packages", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ownerId,
  ...
  async function submitBook(e: FormEvent) {
    ...
    try {
      const phone = bookPhone.replace(/\D/g, "");
      if (phone.length < 9) throw new Error("กรอกเบอร์อย่างน้อย 9 หลัก");
      if (therapists.length > 0 && therapistId == null) throw new Error("กรุณาเลือกนักบำบัด");
      ...
      if (bookPayDue != null && bookPayDue > 0 && !bookSlipUrl.trim()) {
        throw new Error(massagePortalSlipProofMessage(bookPayMode));
      }
      ...
      router.push(massagePublicBookingUrl("", ownerId, bookingId, phone, trial));
    } catch (err) {
  ...
  async function submitBuy(e: FormEvent) {
    ...
    try {
      ...
      const res = await fetch("/api/massage/public/portal/purchase-package", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ownerId,
  ...
  const banner = shop.portalBannerUrl || MASSAGE_PORTAL_SAMPLE_BANNER;
  const title = shop.displayName.trim() || "ร้านนวด";
          ...
          >
            ...
            {therapists.length > 0 ? (
              <div className="space-y-2">
                <p className={formLabelClass}>นักบำบัด</p>
                <div className="flex flex-wrap gap-2" role="listbox" aria-label="เลือกนักบำบัด">
                  {therapists.map((s) => {
                    const active = therapistId === s.id;
                    return (
                      <button
                        key={s.id}
                        type="button"
                ...
                {(() => {
                  const selected = therapists.find((s) => s.id === therapistId);
                  ...
                  return (
                    <p className="text-[11px] font-semibold leading-relaxed text-[#66638c]">
                      ...
                      {massageFormatWorkWeekdaysLabel(selected.workWeekdays ?? [0, 1, 2, 3, 4, 5, 6])}
                    </p>
                  );
                })()}
              </div>
            ...
            <div className="space-y-2">
              ...
              ) : therapists.length > 0 && therapistId == null ? (
                <p className="text-xs font-semibold text-[#66638c]">เลือกนักบำบัดก่อน</p>
              ) : therapistDayOff ? (
                <p className="rounded-xl border border-amber-200/80 bg-amber-50/90 px-3 py-3 text-xs font-semibold text-amber-900">
                  นักบำบัดไม่รับบริการวันนี้ — เลือกวันอื่นหรือนักบำบัดอื่น
                </p>
              ) : availSlots.length === 0 ? (
            ...
            {bookPayDue != null && bookPayDue > 0 ? (
              <div className="space-y-3 rounded-2xl border border-[#5b61ff]/25 bg-[#5b61ff]/5 p-4 sm:max-w-lg">
                ...
                {bookSlipUrl ? (
                  ...
                  <p className="text-xs font-semibold text-amber-800">
                    {massagePortalSlipProofMessage(bookPayMode)}
                  </p>
                )}
              </div>
            ) : null}
      ...
      {buyPkg ? (
        <div className="fixed inset-0 z-[200] flex items-end justify-center bg-[#1e1b4b]/40 p-3 sm:items-center">
          <div
            role="dialog"
            aria-modal
            aria-labelledby="massage-buy-title"
            className={cn(
              appPublicCheckInGlassCardClass,
          ...
          >
            <div className="flex items-start justify-between gap-3">
              <div>
                <h3 id="massage-buy-title" className="text-lg font-black text-[#1e1b4b]">
                  {buyPkg.name}
                </h3>
