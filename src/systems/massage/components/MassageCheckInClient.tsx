...
import {
  ...
  massageModalCameraBackdropClass,
...
const MASSAGE_CHECKIN_STAFF_CARD_MAX = 8;
...
export function MassageCheckInClient({
  embedded = false,
  ...
  const useStaffCards =
    stylists.length > 0 && stylists.length <= MASSAGE_CHECKIN_STAFF_CARD_MAX;
    ...
    try {
      ...
      const res = await fetch("/api/massage/check-in", {
        ...
        body: JSON.stringify({
          ...
          ...(sidCash != null && Number.isInteger(sidCash) && sidCash > 0 ? { therapistId: sidCash } : {}),
        }),
      });
  ...
  return (
    <div className={embedded ? "space-y-4 sm:space-y-5" : massagePageStackClass}>
      <section className={massageSectionFirstClass} aria-label="พนักงานที่บันทึก">
        {stylists.length === 0 ? (
          ...
          <button
            ...
            aria-controls="massage-stylist-modal"
      ...
      <section className={massageSectionNextClass} aria-label="ค้นหาลูกค้า">
      ...
      {(customerName !== null || subs.length > 0 || msg) && (
        <section className={massageSectionNextClass} aria-label="ผลค้นหา">
          <div className="rounded-[2rem] border border-[#e0dcf5] bg-gradient-to-b from-white to-[#faf9ff] p-4 shadow-sm sm:p-5">
            ...
            {subs.length === 0 && phone.length >= 9 && !searching ? (
              <p className={`mt-4 ${massageCardSurfaceRadiusClass} border border-amber-200/90 bg-amber-50/90 px-3 py-2.5 text-sm font-medium text-amber-950`}>
      ...
      <section className={massageSectionNextClass} aria-label="บันทึกด่วน">
      ...
      {stylistModalOpen && !useStaffCards
        ? (
            <MassageModalPortal>
            <div className={massageModalBackdropClass} role="presentation" onClick={() => setStylistModalOpen(false)}>
              <div
                id="massage-stylist-modal"
                role="dialog"
                aria-modal="true"
                aria-labelledby="massage-stylist-modal-title"
      ...
      {cashModalOpen
        ? (
          ...
          <div
            role="dialog"
            aria-modal="true"
            aria-labelledby="massage-cash-modal-title"
            className={massageModalPanelLgClass}
            onClick={(e) => e.stopPropagation()}
          >
            <div className={massageModalHeaderClass}>
              <div className="min-w-0">
                <h2 id="massage-cash-modal-title" className={massageModalTitleClass}>
          ...
          {cashCameraOpen ? (
            <div
              className={massageModalCameraBackdropClass}
              role="dialog"
              aria-modal="true"
      ...
      {msg ?
        <p className={`${massageCardSurfaceRadiusClass} bg-emerald-50 ${massageCardBodyPaddingXClass} py-3 text-sm text-emerald-900`}>{msg}</p>
      : null}
      {err ?
        <p className={`${massageCardSurfaceRadiusClass} bg-red-50 ${massageCardBodyPaddingXClass} py-3 text-sm text-red-800`}>{err}</p>
      : null}
    </div>
  );
}
