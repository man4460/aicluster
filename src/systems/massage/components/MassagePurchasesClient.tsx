...
import { useMassageSubscriptionSaleReceiptBlobUrl } from "@/systems/massage/hooks/use-massage-subscription-sale-receipt-blob-url";
...
type Row = {
  ...
  soldByTherapist: { id: number; name: string } | null;
};
...
const slipThumbClassName =
  `self-start ${massageCardSurfaceRadiusClass} border border-[#e0dcfa]/90 bg-gradient-to-br from-white via-[#faf9ff] to-[#eef2ff]/80 shadow-sm ring-1 ring-[#ecebff]/80 hover:ring-[#4d47b6]/35 sm:h-[4.5rem] sm:w-[4.5rem]`;
...
function MassagePurchaseSlipCell(props: {
  row: Row;
  className: string;
  onOpenLightbox: (src: string) => void;
}) {
  const { row, className, onOpenLightbox } = props;
  const { displaySrc, loading } = useMassageSubscriptionSaleReceiptBlobUrl(row.id, row.saleReceiptImageUrl);
...
function MassageEditSlipPreview(props: {
...
export type MassagePurchasesEmbeddedToolbarApi = {
  openSellModal: () => void;
};

type MassagePurchasesClientProps = {
  embedded?: boolean;
  onEmbeddedToolbar?: (api: MassagePurchasesEmbeddedToolbarApi | null) => void;
};

export function MassagePurchasesClient({
  embedded = false,
  onEmbeddedToolbar,
}: MassagePurchasesClientProps = {}) {
  ...
  const load = useCallback(async () => {
    setErr(null);
    const res = await fetch("/api/massage/subscriptions?limit=150", {
      cache: "no-store",
      credentials: "include",
    });
  ...
  async function submitEdit(e: FormEvent) {
    ...
    try {
      ...
      const res = await fetch(`/api/massage/subscriptions/${editTarget.id}`, {
        method: "PATCH",
  ...
  async function removeRow(r: Row) {
    ...
    const res = await fetch(`/api/massage/subscriptions/${r.id}`, {
      method: "DELETE",
      cache: "no-store",
      credentials: "include",
    });
  ...
  return (
    <div className={embedded ? "space-y-4 sm:space-y-5" : massagePageStackClass}>
      {sellNotice ? (
        <p className={`${massageCardSurfaceRadiusClass} bg-emerald-50 ${massageCardBodyPaddingXClass} py-3 text-sm text-emerald-900`}>
          {sellNotice}
        </p>
      ) : null}
      {err ? <p className={massageInlineAlertErrorClass}>{err}</p> : null}
      {loading ? (
        <>
          {!embedded ? (
            <section className={massageSectionFirstClass} aria-label="เครื่องมือ">
              <div className={cn(massageSectionActionsRowClass, "w-full justify-end sm:ml-auto sm:w-auto")}>
                <MassageDashboardBackLink />
                <button
                  type="button"
                  onClick={openSellModal}
                  className={`app-btn-primary inline-flex min-h-[44px] items-center justify-center ${massageCardSurfaceRadiusClass} px-4 py-2.5 text-center text-sm font-semibold text-white`}
                >
                  ขายแพ็กเกจ
                </button>
              </div>
          ...
          <p className={massageMutedLoadingNoticeClass}>กำลังโหลด…</p>
        </>
      ) : rows.length === 0 ? (
        <section className={massageSectionFirstClass} aria-label="ว่าง">
          {!embedded ? (
            <div className={cn(massageSectionActionsRowClass, "w-full justify-end sm:ml-auto sm:w-auto")}>
              <MassageDashboardBackLink />
              <button
          ...
          <p className={`${massageOffersEmptyStateClass} text-center text-sm text-[#66638c]`}>
            ยังไม่มีการซื้อแพ็ก
          </p>
        </section>
      ) : (
        <>
          <section className={massageSectionFirstClass} aria-label="กรองรายการ">
            <div
              className={cn(
                massageOffersFilterBarClass,
            ...
            >
              <div className="flex min-w-0 items-center gap-2.5">
                <button
                  ...
                  title="เปิดตัวกรอง"
                  className={cn(
                    `flex h-10 w-10 shrink-0 touch-manipulation items-center justify-center ${massageCardSurfaceRadiusClass} bg-gradient-to-br from-[#eef2ff] to-[#e0e7ff] text-[#5b61ff] shadow-sm outline-none ring-1 ring-indigo-100/80 transition hover:from-[#e8ecff] hover:to-[#dde4ff] active:scale-[0.97] focus-visible:ring-2 focus-visible:ring-[#5b61ff]/35`,
              ...
              {!embedded ?
                <div className={cn(massageSectionActionsRowClass, "shrink-0")}>
                  <MassageDashboardBackLink />
                  <button
                    type="button"
                    onClick={openSellModal}
                ...
                >
                  <div className={massageModalHeaderClass}>
                    <div className="min-w-0">
                      <h2 id="purchase-filter-dialog-title" className={massageModalTitleClass}>
                        กรองรายการ
                      </h2>
                      <p className={massageModalSubtitleClass}>เบอร์ · ชื่อ (ว่างได้)</p>
                    </div>
                    <button
                      type="button"
                      onClick={closeFilterPopup}
          ...
          {filteredRows.length === 0 ? (
            <section className={massageSectionNextClass} aria-label="ไม่พบ">
              <p
                className={`${massageCardSurfaceRadiusClass} border border-dashed border-amber-200/90 bg-gradient-to-br from-amber-50/95 via-white to-orange-50/40 ${massageCardBodyPaddingXClass} py-8 text-center text-sm font-medium text-amber-950`}
              >
                ไม่พบรายการ — ปรับตัวกรอง
              </p>
            </section>
          ) : (
            <section className={massageSectionNextClass} aria-label="รายการสมาชิก">
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1 space-y-0.5">
                      ...
                      <p className="text-[11px] text-[#66638c]">
                        ช่างขาย:{" "}
                        <span className="font-medium text-[#2e2a58]">{r.soldByTherapist?.name ?? "—"}</span>
                      </p>
                    </div>
        ...
        <MassageModalPortal>
          <div className={massageModalBackdropClass} role="presentation" onClick={() => closeEditModal()}>
            <div
              role="dialog"
              aria-modal="true"
              aria-labelledby="massage-purchase-edit-title"
              className={massageModalPanelMdClass}
              onClick={(e) => e.stopPropagation()}
            >
              <div className={massageModalHeaderClass}>
                <div className="min-w-0">
                  <h2 id="massage-purchase-edit-title" className={massageModalTitleClass}>
                    แก้ไขสมาชิกแพ็กเกจ
                  </h2>
                  <p className={cn(massageModalSubtitleClass, "truncate tabular-nums")}>{editTarget.customer.phone}</p>
              ...
              <div className={`${massageCardSurfaceRadiusClass} border border-[#ecebff] bg-[#faf9ff] p-3`}>
