...
import { daysInBangkokMonth } from "@/lib/massage/bangkok-day";
...
import { MassageCostToolbarInline } from "@/systems/massage/components/MassageCostsClient";
import { MassageDashboardCharts } from "@/systems/massage/components/MassageDashboardCharts";
import {
...
const massageFinanceFilterSelectClass =
  `w-full ${massageCardSurfaceRadiusClass} border border-[#e8e6f4]/90 bg-gradient-to-br from-white/95 to-[#f8f7ff]/60 px-3 py-2 text-sm font-semibold text-[#1e1b4b] shadow-sm outline-none transition focus:ring-2 focus:ring-violet-500/35`;
const massageFinanceFilterInputClass =
  `w-full ${massageCardSurfaceRadiusClass} border border-[#e8e6f4]/90 bg-gradient-to-br from-white/95 to-[#f8f7ff]/60 px-3 py-2 text-sm font-semibold text-[#1e1b4b] placeholder:text-slate-400 shadow-sm outline-none focus:ring-2 focus:ring-violet-500/35`;

function financeListTabBtnClass(active: boolean, variant: "sales" | "costs") {
  return cn(
    `inline-flex items-center justify-center ${massageCardSurfaceRadiusClass} px-3 py-1.5 text-xs font-bold transition-all duration-200`,
    active &&
      variant === "sales" &&
...
function massageHistoryReceiptImgUrl(src: string | null | undefined): string | null {
  const origin = typeof window !== "undefined" ? window.location.origin : "";
  return normalizeMassageSlipUrlForDashboard(src, origin);
}

export function MassageHistoryClient({
  embedded = false,
  onRequestCostsTab,
  ...
  /** แผงรายจ่าย (MassageCostsClient แบบ hideEmbeddedToolbar) */
  costsPanel?: ReactNode;
  costToolbar?: MassageCostToolbarApi | null;
  ...
  useEffect(() => {
    const onFinanceSales =
      embedded && pathname === "/dashboard/massage/finance";
    const onLegacyHistory = !embedded && pathname === "/dashboard/massage/history";
  ...
  const fetchHistory = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/massage/history?${params.toString()}`);
  ...
  const fetchSpark = useCallback(async () => {
    ...
    try {
      ...
      if (month !== "all" && day !== "all") params.set("day", String(day));
      const res = await fetch(`/api/massage/history/spark?${params.toString()}`);
      const data = (await res.json().catch(() => ({}))) as {
  ...
  async function submitEdit(e: React.FormEvent) {
    ...
    if (editTarget.visitType === "CASH_WALK_IN") {
      ...
      if (editReceiptFile) {
        ...
        const up = await fetch("/api/massage/cash-receipt/upload", { method: "POST", body: fd });
    ...
    setEditSaving(true);
    try {
      const res = await fetch(`/api/massage/history/${editTarget.id}`, {
        method: "PATCH",
  ...
  async function removeLog(l: LogRow) {
    ...
    const res = await fetch(`/api/massage/history/${l.id}`, { method: "DELETE" });
    const data = (await res.json().catch(() => ({}))) as { error?: string };
    if (!res.ok) {
  ...
  const resolvedEditReceipt =
    editTarget && !editReceiptRemoved ? massageHistoryReceiptImgUrl(editTarget.receiptImageUrl) : null;
  ...
  return (
    <div className={embedded ? "min-w-0 space-y-4" : massagePageStackClass}>
      <AppDashboardSection tone="violet">
            ...
            <div className="flex flex-col gap-3 rounded-[2rem] border border-white/55 bg-gradient-to-br from-white/45 via-[#faf9ff]/40 to-[#ecfdf5]/25 p-3 shadow-[0_18px_40px_-24px_rgba(30,27,75,0.35)] backdrop-blur-xl md:hidden sm:p-4">
              ...
              <div className="flex items-center justify-between gap-3">
                ...
                <button
                  ...
                  className={`inline-flex h-9 w-9 shrink-0 items-center justify-center ${massageCardSurfaceRadiusClass} border border-indigo-100/90 bg-gradient-to-br from-[#eef2ff] to-[#e0e7ff] text-violet-600 shadow-sm ring-1 ring-white/60 backdrop-blur-md transition-all active:scale-95`}
        ...
        : <div className="flex items-center justify-between gap-4 rounded-[2rem] border border-white/55 bg-gradient-to-br from-white/45 via-[#faf9ff]/40 to-[#ecfdf5]/25 p-4 shadow-[0_18px_40px_-24px_rgba(30,27,75,0.35)] backdrop-blur-xl sm:p-5">
            ...
            <button
              type="button"
              suppressHydrationWarning
              className={`inline-flex h-9 w-9 shrink-0 items-center justify-center ${massageCardSurfaceRadiusClass} border border-indigo-100/90 bg-gradient-to-br from-[#eef2ff] to-[#e0e7ff] text-violet-600 shadow-sm ring-1 ring-white/60 backdrop-blur-md transition-all active:scale-95 md:hidden`}
              onClick={() => setMobileFilterOpen(true)}
              aria-label="เปิดตัวกรองข้อมูลการเงิน"
            >
              <svg
        ...
        >
          <div className="grid min-w-0 w-full grid-cols-4 gap-3">
            <div className="space-y-1.5">
              ...
              <select
                id="bb-fin-y"
                className={massageFinanceFilterSelectClass}
                value={String(year)}
            ...
            <div className="space-y-1.5">
              ...
              <select
                id="bb-fin-d"
                className={cn(massageFinanceFilterSelectClass, month === "all" && "cursor-not-allowed opacity-60")}
                disabled={month === "all"}
            ...
            <div className="space-y-1.5">
              ...
              <input
                id="bb-fin-q"
                className={massageFinanceFilterInputClass}
        ...
        {sparkLoading ? (
          ...
          <>
            ...
            <div className="mt-4 min-w-0">
              <MassageDashboardCharts visitDualBuckets={sparkVisitDual} packageSalesBuckets={sparkPackageSales} />
            </div>
          </>
        )}
      </AppDashboardSection>
      ...
      {mobileFilterOpen ?
        <MassageModalPortal>
          ...
          >
            <div
              id="massage-finance-filter-dialog"
              role="dialog"
              aria-modal="true"
              aria-labelledby="massage-finance-filter-dialog-title"
              className={massageModalPanelMdClass}
              onClick={(e) => e.stopPropagation()}
            >
              <div className={massageModalHeaderClass}>
                <div className="min-w-0">
                  <h2 id="massage-finance-filter-dialog-title" className={massageModalTitleClass}>
                    ตัวกรองข้อมูลการเงิน
                  </h2>
                  <p className={massageModalSubtitleClass}>เลือกช่วงเวลาและคำค้นหาเพื่อดูผลลัพธ์</p>
                </div>
                <button
                  type="button"
                  onClick={() => setMobileFilterOpen(false)}
              ...
              >
                ...
                <div className="flex flex-col-reverse gap-2 pt-1 sm:flex-row sm:justify-end">
                  ...
                  <button
                    type="submit"
                    className={`app-btn-primary min-h-[48px] ${massageCardSurfaceRadiusClass} px-4 py-3 text-sm font-semibold text-white`}
                  >
                    ใช้การกรอง
                  </button>
                </div>
      ...
      {error ? <p className={massageInlineAlertErrorClass}>{error}</p> : null}
      ...
      <AppDashboardSection tone="slate">
        <div className="flex flex-col gap-4 rounded-[2rem] border border-white/55 bg-gradient-to-br from-white/40 via-[#faf9ff]/35 to-[#fff7ed]/18 p-4 shadow-[0_18px_40px_-24px_rgba(30,27,75,0.35)] backdrop-blur-xl sm:flex-row sm:items-center sm:justify-between sm:p-5">
          ...
          {financeUnified ? (
            ...
            >
              {activeListTab === "costs" ? (
                <MassageCostToolbarInline toolbar={costToolbar} busy={costToolbarBusy} />
              ) : null}
              <button
                type="button"
                suppressHydrationWarning
            ...
            >
              ...
              {onRequestCostsTab ? (
                ...
                <Link
                  href="/dashboard/massage/finance?tab=costs"
                  className={financeListTabBtnClass(false, "costs")}
                >
                  รายจ่าย
                </Link>
        ...
        {activeListTab === "sales" && !loading && logs.length === 0 && !error ? (
          <p className={`${massageOffersEmptyStateClass} mt-4 text-sm text-slate-600`}>
          ...
          >
            <ul className="space-y-2 p-3 pb-4 sm:p-4">
              {logs.map((l) => {
                ...
                const receiptSrc = massageHistoryReceiptImgUrl(l.receiptImageUrl);
                return (
                  <li
                    key={l.id}
                    className={cn(
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1 space-y-0.5">
                      ...
                      <p className="text-[11px] leading-snug text-[#8b87ad]">
                        ...
                        {l.therapistName ? ` · ${l.therapistName}` : null}
                      </p>
                      {l.note ? (
                    ...
                    <div className="flex shrink-0 flex-col items-end gap-1.5 sm:min-w-[5.5rem]">
                      ...
                      <div className={cn(massageIconToolbarGroupClass, "shrink-0")} role="group" aria-label="แก้ไขหรือลบ">
      ...
      {editTarget ? (
        <MassageModalPortal>
          <div className={massageModalBackdropClass} role="presentation" onClick={() => closeEditModal()}>
            <div
              role="dialog"
              aria-modal="true"
              aria-labelledby="massage-history-edit-title"
              className={massageModalPanelMdClass}
              onClick={(e) => e.stopPropagation()}
            >
              <div className={massageModalHeaderClass}>
                <div className="min-w-0">
                  <h2 id="massage-history-edit-title" className={massageModalTitleClass}>
                    แก้ไขรายการ
                  </h2>
                  <p className={cn(massageModalSubtitleClass, "truncate tabular-nums")}>รายการ #{editTarget.id}</p>
                </div>
                <button
                  type="button"
              ...
              {editTarget.visitType === "CASH_WALK_IN" ? (
                <div className={`${massageCardSurfaceRadiusClass} border border-[#ecebff] bg-[#faf9ff] px-3 py-2.5`}>
              ...
              <div className="flex flex-col-reverse gap-2 pt-1 sm:flex-row sm:justify-end">
                ...
                <button
                  type="submit"
                  disabled={editSaving}
                  className={`app-btn-primary min-h-[48px] ${massageCardSurfaceRadiusClass} px-4 py-3 text-sm font-semibold text-white disabled:opacity-60`}
                >
