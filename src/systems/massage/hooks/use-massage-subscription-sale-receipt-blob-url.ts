...
function saleReceiptApiPath(subscriptionId: number): string {
  return `/api/massage/subscriptions/${subscriptionId}/sale-receipt`;
}

/** true ถ้า URL ที่ API/สเตทให้มาคือ endpoint สลิปตามรหัสสมาชิก (รองรับ absolute URL) */
export function isMassageSubscriptionSaleReceiptApiUrl(
 ...
 */
export function useMassageSubscriptionSaleReceiptBlobUrl(
  subscriptionId: number,
  saleReceiptImageUrl: string | null | undefined,
): { displaySrc: string | null; loading: boolean } {
  const useFetch = isMassageSubscriptionSaleReceiptApiUrl(subscriptionId, saleReceiptImageUrl);
  ...
  return {
    displaySrc: normalizeMassageSlipUrlForDashboard(saleReceiptImageUrl, origin),
    loading: false,
  };
}
