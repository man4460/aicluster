/** ชำระตอนจองร้านนวด — แม่แบบร้านตัดผม */

export type MassagePortalBookingPaymentMode = "NONE" | "DEPOSIT" | "FULL";

export function normalizeMassagePortalPaymentMode(
  value: string | null | undefined,
): MassagePortalBookingPaymentMode {
  if (value === "DEPOSIT" || value === "FULL" || value === "NONE") return value;
  return "NONE";
}

/** ยอดที่ต้องชำระตอนจอง — null = ไม่บังคับ */
export function massageComputePortalPayDue(opts: {
  mode: MassagePortalBookingPaymentMode;
...
export function massagePortalSlipProofMessage(mode: MassagePortalBookingPaymentMode): string {
...
export function massageComputeBookingPaymentStatus(
  totalBaht: number,
