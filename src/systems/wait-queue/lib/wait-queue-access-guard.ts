...
export type WaitQueueAccessFailReason = "no_module" | "staff" | "not_subscribed" | "no_plan";

export async function loadWaitQueueAccessState(userId: string): Promise<
  | { ok: true; mod: { id: string; slug: string; groupId: number }; billingUserId: string }
  | { ok: false; reason: WaitQueueAccessFailReason }
> {
  const mod = await prisma.appModule.findFirst({
    where: { slug: WAIT_QUEUE_MODULE_SLUG, isActive: true },
  ...
  if (ctx.isStaff && !STAFF_ALLOWED_MODULE_SLUGS.has(WAIT_QUEUE_MODULE_SLUG)) {
