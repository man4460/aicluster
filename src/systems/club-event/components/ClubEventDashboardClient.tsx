...
import { Calendar, ClipboardCheck, Images, Play, Plus, Settings } from "lucide-react";
...
import {
  ...
  isClubEventDashboardCommitteeLegacyTab,
  parseClubEventDashboardTab,
  type ClubEventDashboardTabKey,
} from "@/systems/club-event/club-event-module-nav";
import { ClubEventDuesStatusPanel } from "@/systems/club-event/components/ClubEventDuesStatusPanel";
...
import type { ClubEventProfileDto, ClubEventRecordDto } from "@/systems/club-event/lib/mappers";
import {
...
export function ClubEventDashboardClient({
  initialProfile,
}: {
  ...
  const tab = parseClubEventDashboardTab(rawTab);
  ...
  useEffect(() => {
    if (!isClubEventDashboardCommitteeLegacyTab(rawTab)) return;
    router.replace(clubEventManageHref("committee"), { scroll: false });
  }, [rawTab, router]);
  ...
  const loadEvents = useCallback(async () => {
    if (isClubEventDashboardCommitteeLegacyTab(rawTab) || tab === "dues") {
      setLoading(false);
      return;
    }
    setLoading(true);
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const res = await fetch("/api/club-event/session/profile", { credentials: "include" });
  ...
  return (
    <>
        ...
        action={
          tab === "dues" ? (
            <Link
              ...
              aria-label="ตั้งค่าค่าบำรุง"
            >
              <Settings className="h-4 w-4 sm:mr-1.5" aria-hidden />
              <span className="hidden sm:inline">ตั้งค่าค่าบำรุง</span>
            </Link>
          ) : (
            <Link
      ...
      >
        {tab === "dues" ? (
          <ClubEventDuesStatusPanel
            duesEnabled={Boolean(profile.duesEnabled)}
            duesLinkId={profile.duesLinkId}
          />
        ) : loading ? (
