...
import { CAR_WASH_PORTAL_SAMPLE_BANNER } from "@/systems/car-wash/lib/portal-media";
import { carWashPublicBookingUrl } from "@/lib/car-wash/public-url";
import {
...
function findFirstFreeRun(
  times: string[],
  busy: Array<{ startMin: number; endMin: number }>,
  need: number,
  slotMinutes: number,
): string[] | null {
  for (let i = 0; i <= times.length - need; i++) {
    const run = times.slice(i, i + need);
    const start = parseHmToMinutes(run[0]!);
    if (start == null) continue;
    let ok = true;
    for (let k = 0; k < need; k++) {
      const tm = parseHmToMinutes(run[k]!);
      if (tm == null || tm !== start + k * slotMinutes) {
        ok = false;
        break;
      }
      for (const b of busy) {
        if (tm < b.endMin && tm + slotMinutes > b.startMin) {
          ok = false;
          break;
        }
      }
...
export function CarWashBookingPortalClient({
  ownerId,
  trialSessionId,
}: {
  ownerId: string;
  ...
  const slotMinutes = shop ? normalizeSlotMinutes(shop.slotMinutes) : 30;
  const slotsNeeded = carWashSlotsNeeded(durationMinutes, slotMinutes);
  ...
  useEffect(() => {
    void fetch(`/api/car-wash/public/portal/info?${qBase}`, { cache: "no-store" })
      .then(async (res) => {
        if (!res.ok) {
        ...
        setShop({
          ...
          slotMinutes: normalizeSlotMinutes(j.shop.slotMinutes ?? 30),
        });
        setPackages(
          (j.packages ?? []).map((p) => ({
            ...p,
            durationMinutes: carWashNormalizeDurationMinutes(p.durationMinutes ?? 30, 30),
          })),
        );
        const st = j.stylists ?? [];
        setStylists(st);
        if (st.length === 1) setStylistId(st[0]!.id);
        setDurationMinutes(normalizeSlotMinutes(j.shop.slotMinutes ?? 30));
  ...
  useEffect(() => {
    ...
    void fetch(`/api/car-wash/public/portal/availability?${q}`, { cache: "no-store" })
      .then(async (res) => {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const res = await fetch("/api/car-wash/public/portal/promptpay-qr", {
          method: "POST",
  ...
  function applyDurationAndAutoSelect(nextDuration: number) {
    const dur = carWashNormalizeDurationMinutes(nextDuration, slotMinutes);
    setDurationMinutes(dur);
    const need = carWashSlotsNeeded(dur, slotMinutes);
    const times = availSlots.map((s) => s.startTime);
    const busy: Array<{ startMin: number; endMin: number }> = [];
    for (const s of availSlots) {
      if (s.available) continue;
      const sm = parseHmToMinutes(s.startTime);
      if (sm == null) continue;
      busy.push({ startMin: sm, endMin: sm + slotMinutes });
    }
    const run = findFirstFreeRun(times, busy, need, slotMinutes);
  ...
  async function lookupMember() {
    ...
    try {
      ...
      const res = await fetch("/api/car-wash/public/portal/member-packages", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ownerId,
  ...
  async function submitBook(e: FormEvent) {
    ...
    try {
      const phone = bookPhone.replace(/\D/g, "");
      if (phone.length < 9) throw new Error("กรอกเบอร์อย่างน้อย 9 หลัก");
      if (stylists.length > 0 && stylistId == null) throw new Error("กรุณาเลือกจุดบริการ");
      ...
      const payDue = pkg?.payDueBaht ?? null;
      if (payDue != null && payDue > 0 && !bookSlipUrl) {
        throw new Error(
          shop?.portalBookingPaymentMode === "FULL"
            ? "กรุณาอัปโหลดสลิปชำระเต็มยอด"
            : "กรุณาอัปโหลดสลิปมัดจำ",
        );
      }
      const startTime = selectedSlots[0]!;
      const scheduledLocal = `${bookDate}T${startTime}`;
      const res = await fetch("/api/car-wash/public/portal/book", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ownerId,
          ...
          plateNumber: bookPlate.trim() || undefined,
          paymentMethod: payDue != null && payDue > 0 ? bookPayMethod : "UNPAID",
          amountPaidBaht: payDue != null && payDue > 0 ? payDue : 0,
          paymentSlipUrl: bookSlipUrl || undefined,
        }),
      });
      const j = (await res.json().catch(() => ({}))) as {
        error?: string;
      ...
      router.push(carWashPublicBookingUrl("", ownerId, bookingId, phone, trial));
    } catch (err) {
  ...
  async function uploadSlip(file: File) {
    ...
    const res = await fetch("/api/car-wash/public/portal/upload-slip", { method: "POST", body: fd });
  ...
  async function submitBuy(e: FormEvent) {
    ...
    try {
      ...
      const res = await fetch("/api/car-wash/public/portal/purchase-package", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ownerId,
  ...
  const banner = shop.portalBannerUrl || CAR_WASH_PORTAL_SAMPLE_BANNER;
  const title = shop.displayName.trim() || "คาร์แคร์";
        ...
        <section id="book" className="scroll-mt-16">
          ...
          >
            ...
            <label className="block sm:max-w-md">
              <span className={formLabelClass}>ทะเบียนรถ (ไม่บังคับ)</span>
              <input
                value={bookPlate}
                onChange={(e) => setBookPlate(e.target.value.slice(0, 32))}
                className={fieldClass}
            ...
            {(() => {
              const pkg = packages.find((p) => p.id === selectedPackageId);
              const payDue = pkg?.payDueBaht ?? null;
              if (payDue == null || payDue <= 0) return null;
              return (
                <div className="space-y-3 rounded-2xl border border-[#5b61ff]/25 bg-[#5b61ff]/5 p-4 sm:max-w-lg">
                  <p className="text-sm font-black text-[#1e1b4b]">
                    {shop?.portalBookingPaymentMode === "FULL" ? "ชำระเต็มยอด" : "มัดจำ"} ฿
                    {payDue.toLocaleString("th-TH")}
                  </p>
                  <div className="flex flex-wrap gap-2">
                    <button
                      type="button"
                  ...
                  <AppGalleryCameraFileInputs
                    ...
                    cameraInputRef={bookSlipGalleryRef}
                    onChange={(e) => {
                      const file = e.target.files?.[0];
                      e.target.value = "";
                      if (file) void uploadBookSlip(file).catch((err) => setBookErr(err instanceof Error ? err.message : "อัปโหลดไม่สำเร็จ"));
                    }}
                  />
                  <AppImagePickCameraButtons
      ...
      {buyPkg ? (
        <div className="fixed inset-0 z-[200] flex items-end justify-center bg-[#1e1b4b]/40 p-3 sm:items-center">
          <div
            role="dialog"
            aria-modal
            aria-labelledby="car-wash-buy-title"
            className={cn(
              appPublicCheckInGlassCardClass,
          ...
          >
            <div className="flex items-start justify-between gap-3">
              <div>
                <h3 id="car-wash-buy-title" className="text-lg font-black text-[#1e1b4b]">
                  {buyPkg.name}
                </h3>
