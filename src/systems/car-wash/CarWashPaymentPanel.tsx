...
export type CarWashPaymentPanelProps = {
  amountBaht: number;
  method: CarWashPaymentMethod;
  slipUrl: string | null;
  onMethodChange: (method: CarWashPaymentMethod) => void;
    ...
    >
      <div>
        ...
        <div className="mt-2 flex flex-wrap gap-2" role="group" aria-label="ช่องทางชำระเงิน">
          {CAR_WASH_PAYMENT_METHODS.map((m) => (
            <button
              key={m}
              type="button"
              disabled={disabled}
              ...
              className={cn(
                ...
                method === m ? carWashPaymentChipActiveClass : carWashPaymentChipIdleClass,
              )}
              aria-pressed={method === m}
            >
              {carWashPaymentMethodLabel(m)}
            </button>
          ))}
        </div>
      </div>
        ...
        <div className="space-y-2 rounded-[1.25rem] border border-[#e8e6fc]/90 bg-white/80 p-3">
          ...
          {info?.qrDataUrl ? (
            <button
              type="button"
              disabled={disabled}
              className={cn(carWashPaymentCtaClass, "w-full min-h-[44px] rounded-2xl text-sm")}
        ...
        ? createPortal(
            <div className="fixed inset-0 z-[230] flex items-center justify-center p-4 sm:p-6" role="presentation">
              ...
              <div
                role="dialog"
                aria-modal="true"
                aria-labelledby="car-wash-customer-pp-qr-title"
              ...
              >
                <p
                  id="car-wash-customer-pp-qr-title"
