...
/**
 * Pub/sub ในโปรเซสเดียวกันสำหรับลานล้างวันนี้ (SSE)
 * single Node / next start — หลาย instance ต้องใช้ Redis ภายหลัง
 */
const listenersByOwner = new Map<string, Set<Listener>>();

export function subscribeCarWashLaneBoard(ownerUserId: string, listener: Listener): () => void {
...
export function publishCarWashLaneBoard(ownerUserId: string): void {
