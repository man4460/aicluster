...
import { DORMITORY_ROOMS_HREF } from "@/systems/dormitory/dormitory-module-nav";
import {
  createDormIncomeCategory,
  createDormIncomeEntry,
  deleteDormIncomeCategory,
  deleteDormIncomeEntry,
  fetchDormIncomeCategories,
  fetchDormIncomeEntries,
  updateDormIncomeCategory,
  updateDormIncomeEntry,
  uploadDormIncomeSlip,
  type DormIncomeCategory,
  type DormIncomeEntry,
} from "@/systems/dormitory/dorm-income-client";
import { dormBtnPrimary } from "@/systems/dormitory/dorm-ui";
import { dormFieldClass, dormFilterChipClass, dormListRowCardClass } from "@/systems/dormitory/dorm-ui-tokens";
import {
...
function paymentSortDay(r: PaymentRow): string {
  if (r.paymentStatus === "PAID" && r.paidAt) return bangkokCalendarDay(r.paidAt);
  return bangkokCalendarDay(r.updatedAt);
}
...
export function DormFinanceHistoryPanel({
  keyword,
  dateFrom,
  dateTo,
  financeRangeLabel,
  ...
  const [incomes, setIncomes] = useState<DormIncomeEntry[]>([]);
  const [incomeCategories, setIncomeCategories] = useState<DormIncomeCategory[]>([]);
  const [loading, setLoading] = useState(true);
  const [err, setErr] = useState<string | null>(null);
  const [incomeFilterCat, setIncomeFilterCat] = useState<"all" | "TENANT_RENT" | string>("all");

  const [incomeOpen, setIncomeOpen] = useState(false);
  const [incomeBusy, setIncomeBusy] = useState(false);
  const [incomeEditing, setIncomeEditing] = useState<DormIncomeEntry | null>(null);
  ...
  const [incomeCatEdit, setIncomeCatEdit] = useState<DormIncomeCategory | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    setErr(null);
    try {
      const [payRes, incomeList, cats] = await Promise.all([
        fetch("/api/dorm/payments/history", { cache: "no-store", credentials: "include" }),
        fetchDormIncomeEntries(dateFrom, dateTo),
        fetchDormIncomeCategories(),
      ]);
      const payData = (await payRes.json().catch(() => ({}))) as { items?: PaymentRow[]; error?: string };
      if (!payRes.ok) throw new Error(payData.error ?? "โหลดประวัติชำระไม่สำเร็จ");
  ...
  const filteredPayments = useMemo(() => {
    ...
    return list.filter(
      (r) =>
        r.bill.room.roomNumber.toLowerCase().includes(kw) ||
        r.tenant.name.toLowerCase().includes(kw) ||
        r.tenant.phone.replace(/\D/g, "").includes(kw.replace(/\D/g, "")) ||
        String(r.bill.billingMonth).includes(kw) ||
        String(r.bill.billingYear).includes(kw),
    );
  }, [payments, dateFrom, dateTo, kw]);

  const filteredHistoryRows = useMemo(() => {
    type Row =
      | { key: string; sortAt: string; kind: "payment"; payment: PaymentRow }
      | { key: string; sortAt: string; kind: "income"; income: DormIncomeEntry };

    const rows: Row[] = [];
    const showPayments = incomeFilterCat === "all" || incomeFilterCat === "TENANT_RENT";
    const showIncomes = incomeFilterCat !== "TENANT_RENT";

    if (showPayments) {
      for (const p of filteredPayments) {
        if (p.paymentStatus !== "PAID" || !p.paidAt) continue;
        rows.push({
          key: `pay-${p.id}`,
          sortAt: p.paidAt,
          kind: "payment",
  ...
  }, [filteredPayments, incomes, incomeFilterCat, kw]);

  function resetIncomeForm() {
    setIncomeEditing(null);
    const preferred =
      incomeFilterCat !== "all" &&
      incomeFilterCat !== "TENANT_RENT" &&
  ...
  function openIncomeEdit(row: DormIncomeEntry) {
  ...
  async function uploadIncomeSlip(file: File) {
    ...
    setErr(null);
    try {
      const url = await uploadDormIncomeSlip(file);
  ...
  async function submitIncome() {
    ...
    try {
      ...
      if (incomeEditing) {
        await updateDormIncomeEntry(incomeEditing.id, payload);
      } else {
        await createDormIncomeEntry(payload);
      }
      setIncomeOpen(false);
      resetIncomeForm();
      await load();
  ...
  async function deleteIncome(row: DormIncomeEntry) {
    const ok = await notice.confirm(`ลบรายรับ «${row.label}» ใช่หรือไม่?`);
    if (!ok) return;
    setErr(null);
    try {
      await deleteDormIncomeEntry(row.id);
      await load();
      onRefreshFinance?.();
    } catch (e) {
  ...
  function openIncomeCatEdit(c: DormIncomeCategory) {
  ...
  async function submitIncomeCategory() {
    ...
    try {
      ...
      if (incomeCatEdit) {
        await updateDormIncomeCategory(incomeCatEdit.id, name);
      } else {
        await createDormIncomeCategory(name);
      }
      setIncomeCatFormOpen(false);
      setIncomeCatEdit(null);
      setIncomeCatName("");
  ...
  async function deleteIncomeCategory(c: DormIncomeCategory) {
    ...
    setIncomeCatErr(null);
    try {
      await deleteDormIncomeCategory(c.id);
  ...
  const tenantRentLabel =
    incomeCategories.find((c) => c.kind === "TENANT_RENT")?.name ?? "ค่าเช่า / หอพัก";
  ...
  return (
    <>
      ...
      <p className="mt-2 text-xs font-semibold text-[#66638c]">
        ...
        {" · "}
        รายรับค่าเช่า = ชำระแล้วเท่านั้น
        {" · "}
        <Link href={DORMITORY_ROOMS_HREF} className="font-bold text-[#4338ca] underline-offset-2 hover:underline">
          รับชำระ / ดูค้างชำระที่ห้อง
        </Link>
      </p>
      ...
      >
        <div className="flex w-max touch-pan-x gap-2 pr-1 sm:flex-wrap sm:pr-0">
          <button
            ...
            className={cn("shrink-0 snap-start transition", dormFilterChipClass(incomeFilterCat === "all"))}
          ...
          <button
            type="button"
            onClick={() => setIncomeFilterCat("TENANT_RENT")}
            className={cn("shrink-0 snap-start transition", dormFilterChipClass(incomeFilterCat === "TENANT_RENT"))}
            aria-pressed={incomeFilterCat === "TENANT_RENT"}
          >
            {tenantRentLabel}
          </button>
          {customIncomeCategories.map((c) => (
            <button
              ...
              className={cn("shrink-0 snap-start transition", dormFilterChipClass(incomeFilterCat === c.id))}
      ...
      {loading ? (
        ...
        <AppEmptyState tone="slate" className="mt-4">
          {customIncomeCategories.length === 0 && incomeFilterCat !== "TENANT_RENT"
            ? "เริ่มจากเพิ่มหมวด แล้วเพิ่มรายรับ — หรือดูค่าเช่าจากการชำระ"
          ...
          <ul className="space-y-2 pr-0.5">
            {filteredHistoryRows.map((row) => {
              if (row.kind === "income") {
                ...
                return (
                  <li key={row.key} className={cn(dormListRowCardClass, "flex items-start gap-2")}>
                    {slip ? (
                      <AppImageThumb
                        src={slip}
                        alt={`สลิป ${item.label}`}
                    ...
                    <div className="flex shrink-0 flex-col items-end gap-1.5">
                      <p className="text-lg font-black tabular-nums text-emerald-700">
                        ฿{formatDormAmountStable(item.amountBaht)}
                      </p>
              ...
              const p = row.payment;
              const slip = p.proofSlipUrl?.trim() || "";
              return (
                <li key={row.key} className={cn(dormListRowCardClass, "flex items-start gap-2")}>
                  {slip ? (
                    <AppImageThumb
                      src={slip}
                      alt={`สลิปห้อง ${p.bill.room.roomNumber}`}
                      onOpen={() => slipLb.open(slip)}
                      className="h-14 w-14 shrink-0"
                    />
                  ) : null}
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-black text-[#1e1b4b]">
                      ห้อง {p.bill.room.roomNumber} · {p.tenant.name}
                    </p>
                    <p className="mt-0.5 text-xs font-semibold text-[#66638c]">
                      งวด {p.bill.billingMonth}/{p.bill.billingYear} · ชำระแล้ว
                    </p>
                    <p className="mt-0.5 text-[11px] font-bold text-[#4d47b6]">
                      {tenantRentLabel} · {formatBangkokDateTimeStable(p.paidAt!)}
                    </p>
                    {p.note ? (
                      <p className="mt-0.5 text-xs font-semibold text-[#66638c]">{p.note}</p>
                    ) : null}
                  </div>
                  <p className="shrink-0 text-lg font-black tabular-nums text-emerald-700">
                    ฿{formatDormAmountStable(p.amountToPay)}
                  </p>
                </li>
              );
            })}
      ...
      >
        <div className="space-y-3">
          ...
          <p className="text-[11px] font-semibold text-[#8b87b8]">
            {tenantRentLabel}มาจากการชำระ — เพิ่มรายรับอื่นผ่านหมวดที่สร้างเอง
          </p>
          <label className="block text-left text-sm font-bold text-[#1e1b4b]">
            รายละเอียดรายการ
            <input
              ...
              placeholder="เช่น ค่าจอดรถ · ขายของฝาก"
            />
          </label>
          <label className="block text-left text-sm font-bold text-[#1e1b4b]">
            จำนวนเงิน (บาท)
          ...
          incomeCatFormOpen ? (
            ...
            <div className="flex justify-end gap-2">
              <button
                ...
                className={cn(dormBtnPrimary, "rounded-[1rem] px-4 py-2 text-sm font-bold")}
              >
                + เพิ่มหมวดหมู่
              </button>
              <button
        ...
        {incomeCatFormOpen ? (
          <label className="block text-left text-sm font-bold text-[#1e1b4b]">
            ...
            <input
              ...
              placeholder="เช่น ค่าจอดรถ · รายรับอื่น"
              autoFocus
            />
          </label>
        ) : (
