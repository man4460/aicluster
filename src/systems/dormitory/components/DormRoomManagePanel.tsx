...
export function DormRoomManagePanel({
  rooms,
  roomHref,
  onRoomClick,
}: {
  rooms: DormRoomManageRow[];
      ...
      {rooms.length === 0 ? (
        ...
        <ul className="grid grid-cols-1 gap-2.5 sm:grid-cols-2 sm:gap-3 lg:grid-cols-3 xl:grid-cols-4">
          {rooms.map((r) => (
            <li key={r.id}>
              {onRoomClick ? (
                <button type="button" className={`${dormRoomListCard} w-full text-left`} onClick={() => onRoomClick(r.id)}>
              ...
              <Link href={roomHref ? roomHref(r.id) : `/dashboard/dormitory/rooms/${r.id}`} className={dormRoomListCard}>
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0 flex-1 space-y-1">
                    <span className={dormRoomFieldLabel}>เลขห้อง</span>
                    <p className={dormRoomNumberList}>{r.roomNumber}</p>
