...
import {
  DormFinanceSubTabs,
  type DormFinancePanel,
} from "@/systems/dormitory/components/DormFinanceSubTabs";
import { DormFinanceHistoryPanel } from "@/systems/dormitory/components/DormFinanceHistoryPanel";
import {
  CostToolbarButtons,
  DormCostsClient,
} from "@/systems/dormitory/components/DormCostsClient";
...
import { fetchDormCostCategories } from "@/systems/dormitory/dorm-cost-client";
import {
  DORMITORY_FINANCE_HREF,
} from "@/systems/dormitory/dormitory-module-nav";import {
  dormFieldClass,
  dormFilterChipClass,
  dormFinanceStatTailClass,
  dormFinanceStatsGridClass,
} from "@/systems/dormitory/dorm-ui-tokens";
import { dormBtnSecondary } from "@/systems/dormitory/dorm-ui";
import { dormSectionRadiusClass } from "@/systems/dormitory/lib/ui-tokens";

type FinanceRange = "TODAY" | "MONTH" | "YEAR" | "CUSTOM";

function parsePanel(raw: string | null): DormFinancePanel {
...
export function DormFinanceClient({ baseUrl }: Props) {
  ...
  const setPanel = useCallback(
    (next: DormFinancePanel) => {
      const href =
        next === "history" ? DORMITORY_FINANCE_HREF : `${DORMITORY_FINANCE_HREF}?panel=${next}`;
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <section aria-label={`สรุปการเงิน ${financeRangeLabel}`}>
        <ul className={dormFinanceStatsGridClass}>
          <li className="rounded-[1.5rem] border border-white/55 bg-white/50 px-3 py-3 shadow-sm ring-1 ring-inset ring-white/40">
            ...
            <p className="mt-2 text-left text-2xl font-black tabular-nums text-emerald-700 sm:text-3xl">
              ฿{formatDormAmountStable(totalRevenue)}
            </p>
          </li>
          <li className="rounded-[1.5rem] border border-white/55 bg-white/50 px-3 py-3 shadow-sm ring-1 ring-inset ring-white/40">
            <p className="text-left text-[10px] font-black uppercase tracking-widest text-[#66638c]">
            ...
            <p className="mt-2 text-left text-2xl font-black tabular-nums text-rose-600 sm:text-3xl">
              ฿{formatDormAmountStable(totalCost)}
            </p>
          </li>
          <li
            className={cn(
          ...
          >
            ...
            <p className="mt-2 text-left text-2xl font-black tabular-nums text-[#1e1b4b] sm:text-3xl">
              ฿{formatDormAmountStable(totalRevenue - totalCost)}
            </p>
          </li>
        </ul>
      </section>
      ...
      <AppDashboardSection tone="violet" className={dormSectionRadiusClass}>
          ...
          action={
            <div className="flex shrink-0 flex-nowrap items-center gap-1.5 sm:gap-2">
              <button
                ...
                aria-controls="dorm-finance-filter-panel"
              ...
              <button
                ...
                aria-controls="dorm-finance-charts"
        ...
        >
          ...
          <div className={cn("grid gap-3", filtersActive ? "sm:grid-cols-12" : undefined)}>
            <label className={cn("min-w-0", filtersActive ? "sm:col-span-9" : undefined)}>
              <span className="sr-only">ค้นหาห้อง ผู้พัก หรืองวด</span>
              <input
                value={keyword}
                onChange={(e) => setKeyword(e.target.value)}
                placeholder="ค้นหาห้อง ผู้พัก หรืองวด…"
                aria-label="ค้นหาห้อง ผู้พัก หรืองวด"
                inputMode="search"
                className={cn(dormFieldClass, "mt-0")}
              />
            </label>
            {filtersActive ? (
        ...
        {chartsOpen ? (
          <div id="dorm-finance-charts" className="mt-4 space-y-4">
            ...
            {financeLoading ? (
              ...
              <AppSparkChartPanel className="w-full min-w-0">
                <AppRevenueCostColumnChart
                  ...
                  formatTitle={(b) =>
                    `รายได้ ${formatDormAmountStable(b.revenue)} · ต้นทุน ${formatDormAmountStable(b.cost)}`
                  }
                />
              </AppSparkChartPanel>
            )}
        ...
        <div className="mt-4 space-y-4 border-t border-[#ecebff] pt-4">
          <DormFinanceSubTabs panel={panel} onPanelChange={setPanel} />

          {panel === "history" ? (
            <div id="dorm-finance-panel-history" role="tabpanel" aria-labelledby="dorm-finance-tab-history">
              <DormFinanceHistoryPanel
                keyword={keyword}
                dateFrom={dateFrom}
                dateTo={dateTo}
          ...
          {panel === "expenses" ? (
            <div id="dorm-finance-panel-expenses" role="tabpanel" aria-labelledby="dorm-finance-tab-expenses">
              <AppSectionHeader
                tone="slate"
                title="รายจ่าย"
                className="flex flex-row items-start justify-between gap-3 sm:items-center"
                actionWrapClassName="shrink-0 self-start pt-0.5 sm:pt-0"
                action={
                  <CostToolbarButtons toolbar={costToolbar} busy={costBusy} compact />
                }
              />
              ...
              >
                <div className="flex w-max touch-pan-x gap-2 pr-1 sm:flex-wrap sm:pr-0">
                  <button
                    ...
                    className={cn("shrink-0 snap-start transition", dormFilterChipClass(costFilterCat === "all"))}
                  ...
                  {costCategories.map((c) => (
                    <button
                      ...
                      className={cn("shrink-0 snap-start transition", dormFilterChipClass(costFilterCat === c.id))}
