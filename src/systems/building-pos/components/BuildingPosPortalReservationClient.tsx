...
export function BuildingPosPortalReservationClient({
  ownerId,
  reservationId,
  phone,
  trialSessionId,
  ...
  const homeHref =
    trialSessionId && trialSessionId !== "prod"
      ? `/building-pos/${ownerId}?t=${encodeURIComponent(trialSessionId)}`
      : `/building-pos/${ownerId}`;
  ...
  useEffect(() => {
    ...
    setBusy(true);
    setErr(null);
    void fetch(`/api/building-pos/public/portal/reservation?${q}`, { cache: "no-store" })
      .then(async (res) => {
              ...
              {reservation.items.length > 0 ? (
                <ul className="mt-3 space-y-1 border-t border-[#ecebff] pt-3 text-sm">
                  {reservation.items.map((it) => (
                    <li key={it.menuItemId} className="flex justify-between gap-2 font-semibold text-[#1e1b4b]">
                      <span>
                        {it.name} × {it.qty}
                      </span>
