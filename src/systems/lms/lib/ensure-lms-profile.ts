...
type Db = Pick<PrismaClient, "lmsProfile" | "user">;
...
function defaultSlugFromUser(username: string, ownerUserId: string): string {
  ...
  return `lms-${ownerUserId.slice(0, 8)}`;
}

export async function ensureLmsProfile(
  db: Db,
  ownerUserId: string,
  trialSessionId: string = TRIAL_PROD_SCOPE,
) {
  const existing = await db.lmsProfile.findUnique({
  ...
  const username = user?.username ?? "lms";
  let slug = defaultSlugFromUser(username, ownerUserId);
  let suffix = 0;
  while (
    await db.lmsProfile.findFirst({
  ...
  try {
    return await db.lmsProfile.create({
      data: {
        ownerUserId,
        trialSessionId,
        slug,
    ...
    if (isPrismaUniqueViolation(e)) {
      const raced = await db.lmsProfile.findUnique({
        where: { ownerUserId_trialSessionId: { ownerUserId, trialSessionId } },
      });
