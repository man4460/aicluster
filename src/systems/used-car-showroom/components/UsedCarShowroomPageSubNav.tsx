...
export type UsedCarShowroomPageSubNavItem = {
  key: string;
  label: string;
 ...
 */
export function UsedCarShowroomPageSubNav({
  title,
  titleIcon,
  titleTone = "sky",
  subtitle,
  ...
  title: string;
  titleIcon?: ReactNode;
  titleTone?: UsedCarShowroomCardTone;
  subtitle?: string;
  items?: UsedCarShowroomPageSubNavItem[];
  activeKey?: string;
    ...
    mobileSelect === false
      ...
      : mobileSelect ??
        (hasTabs && (items?.length ?? 0) >= 2
          ? { id: `used-car-showroom-subnav-${autoId}`, label: ariaLabel ?? "เลือกเมนู" }
          : null);
      ...
      <div className={cn(usedCarShowroomPanelSectionClass, "print:hidden")}>
        <div className="flex flex-nowrap items-center justify-between gap-2">
          ...
          >
            {hasTabs ? (
              useMobileSelect ? (
                <div className="hidden sm:block">
                  <nav
                    className={usedCarShowroomInlineSubNavShellClass}
                    role="tablist"
                    ...
                    {items!.map((item) => {
                      ...
                      return (
                        <button
                          ...
                          className={usedCarShowroomInlineSubNavBtnClass(active)}
                        >
                          {item.icon ? (
                ...
                <nav className={usedCarShowroomInlineSubNavShellClass} role="tablist" aria-label={ariaLabel ?? "เมนูย่อย"}>
                  {items!.map((item) => {
                    const active = activeKey === item.key;
            ...
            {hasTabs && action ? (
              <span className={cn(usedCarShowroomNavDividerClass, useMobileSelect && "hidden sm:block")} aria-hidden />
            ) : null}
            {action}
          </div>
        </div>
        ...
        {useMobileSelect && autoMobileSelect ? (
          <div className="mt-3 w-full sm:hidden">
            ...
            <select
              ...
              className={usedCarShowroomMobileSelectClass}
...
export function UsedCarShowroomPageBlock({
  title,
  titleIcon,
  action,
  children,
  ...
  return (
    <div className={cn(!first && cn(usedCarShowroomPanelDividerClass, "mt-4 pt-4"), className)}>
      {title || action ? (
        <div className="mb-3 flex flex-row flex-wrap items-start justify-between gap-2 sm:gap-3">
          {title ? (
            <h3 className={usedCarShowroomSectionHeadingClass}>
              {titleIcon ? (
                <span
