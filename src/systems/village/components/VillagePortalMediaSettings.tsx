...
const villageFormLabelClass = "text-xs font-black text-[#4d47b6]";
...
export function VillagePortalMediaSettings({
  bannerUrl,
  gallery,
  address,
  contactLine,
  ...
  const bannerCamera = useAppCameraCapture({ title: "ถ่ายแบนเนอร์โครงการ" });
  const galleryCamera = useAppCameraCapture({ title: "ถ่ายรูปโครงการ" });
  ...
  async function uploadFile(file: File): Promise<string> {
    ...
    const res = await fetch("/api/village/upload", { method: "POST", body: fd, credentials: "include" });
  ...
  async function onPickGallery(e: ChangeEvent<HTMLInputElement>) {
    ...
    const slots = VILLAGE_PORTAL_GALLERY_MAX - gallery.length;
    if (slots <= 0) {
      setErr(`อัปโหลดได้ไม่เกิน ${VILLAGE_PORTAL_GALLERY_MAX} รูป`);
      return;
    }
    setUploadBusy(true);
    setErr(null);
    try {
      ...
      onGalleryChange([...gallery, ...added].slice(0, VILLAGE_PORTAL_GALLERY_MAX));
    } catch (e2) {
  ...
  const previewBanner = bannerUrl.trim() || VILLAGE_PORTAL_SAMPLE_BANNER;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        <p className={villageFormLabelClass}>แบนเนอร์เว็บลูกค้า</p>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {!bannerUrl.trim() ? (
            <button
              ...
              disabled={busy}
              onClick={() => {
                onBannerUrlChange(VILLAGE_PORTAL_SAMPLE_BANNER);
                setBannerBroken(false);
              }}
      ...
      <div className="space-y-2">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <p className={villageFormLabelClass}>
            แกลเลอรี ({gallery.length}/{VILLAGE_PORTAL_GALLERY_MAX})
          </p>
        ...
        {gallery.length > 0 ? (
          ...
          <ul className="grid grid-cols-3 gap-2 lg:grid-cols-6">
            {VILLAGE_PORTAL_SAMPLE_GALLERY.map((url, idx) => (
              <li key={`sample-${idx}`}>
                <button
                  type="button"
                  disabled={busy}
                  onClick={() => onGalleryChange([...VILLAGE_PORTAL_SAMPLE_GALLERY])}
        ...
        <div className="flex flex-wrap gap-2">
          <AppImagePickCameraButtons
            disabled={busy || gallery.length >= VILLAGE_PORTAL_GALLERY_MAX}
            busy={uploadBusy}
            onPickGallery={() => galleryPickRef.current?.click()}
            onPickCamera={() =>
              galleryCamera.openCamera(async (file) => {
                if (gallery.length >= VILLAGE_PORTAL_GALLERY_MAX) {
                  setErr(`อัปโหลดได้ไม่เกิน ${VILLAGE_PORTAL_GALLERY_MAX} รูป`);
                  return;
                }
                setUploadBusy(true);
                ...
                try {
                  ...
                  onGalleryChange([...gallery, url].slice(0, VILLAGE_PORTAL_GALLERY_MAX));
                } catch (e2) {
                  setErr(e2 instanceof Error ? e2.message : "อัปโหลดไม่สำเร็จ");
                } finally {
                  setUploadBusy(false);
      ...
      <label className="block space-y-1">
        <span className={villageFormLabelClass}>LINE ID / ลิงก์ LINE</span>
        <input
      ...
      <label className="block space-y-1">
        <span className={villageFormLabelClass}>Facebook</span>
        <input
          className={villageField}
          value={facebookUrl}
          disabled={disabled}
      ...
      <label className="block space-y-1">
        <span className={villageFormLabelClass}>แผนที่ (ลิงก์ Google Maps)</span>
        <input
          className={villageField}
          value={mapUrl}
          disabled={disabled}
      ...
      <label className="block space-y-1">
        <span className={villageFormLabelClass}>ที่อยู่ (แสดงบนเว็บลูกค้า)</span>
        <textarea
