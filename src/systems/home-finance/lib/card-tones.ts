...
/** โทนสีการ์ดรายการโมดูลบันทึกส่วนตัว — เส้นซ้าย + พื้นอ่อน + ไอคอน */
export type HomeFinanceCardTone =
  | "sky"
  | "violet"
  | "emerald"
  | "rose"
...
export type HomeFinanceCardToneClasses = {
  leftBorder: string;
  bg: string;
  iconBg: string;
  iconText: string;
...
const TONES: Record<HomeFinanceCardTone, HomeFinanceCardToneClasses> = {
  sky: {
...
export function homeFinanceCardToneClasses(tone: HomeFinanceCardTone): HomeFinanceCardToneClasses {
  return TONES[tone];
}

/** เปลือกการ์ดรายการ — เส้นซ้าย + พื้นโทน */
export function homeFinanceTonedRowCardClass(tone: HomeFinanceCardTone): string {
  const t = homeFinanceCardToneClasses(tone);
  return cn(
    "flex items-center justify-between gap-3 rounded-lg border border-slate-200/70 border-l-[3px] p-3 shadow-sm transition sm:p-3.5",
    t.leftBorder,
    t.bg,
    t.hoverShadow,
  );
...
export function homeFinanceCardIconTileClass(
...
export function homeFinanceDocumentCardTone(isPdf: boolean): HomeFinanceCardTone {
  return isPdf ? "rose" : "sky";
}

export function homeFinanceCategoryCardTone(isActive: boolean): HomeFinanceCardTone {
  return isActive ? "violet" : "slate";
}
