...
import {
  fetchGeneralStorePosCategories,
  fetchGeneralStorePosProducts,
  fetchGeneralStorePosSales,
  generalStorePosFetchErrorMessage,
  type GeneralStorePosCategoryRow,
  type GeneralStorePosProductRow,
  type GeneralStorePosSaleRow,
} from "@/systems/general-store-pos/lib/client-data";
import { formatThb } from "@/systems/inventory/lib/inventory-client-data";
import { suggestGeneralStorePosStockImageUrl } from "@/systems/general-store-pos/lib/suggest-stock-image";
...
const GENERAL_STORE_CARD_DOUBLE_TAP_MS = 280;
...
export function GeneralStorePosDashboardClient() {
  const [categories, setCategories] = useState<GeneralStorePosCategoryRow[]>([]);
  const [products, setProducts] = useState<GeneralStorePosProductRow[]>([]);
  const [sales, setSales] = useState<GeneralStorePosSaleRow[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [filterCat, setFilterCat] = useState<string | "all">("all");

  const [catModalOpen, setCatModalOpen] = useState(false);
  const [catEdit, setCatEdit] = useState<GeneralStorePosCategoryRow | null>(null);
  ...
  const [prodEdit, setProdEdit] = useState<GeneralStorePosProductRow | null>(null);
  ...
  const setMobileDraftSlot = useGeneralStorePosMobileDraftSlot();
  ...
  const reload = useCallback(async () => {
    ...
    try {
      ...
      setError(generalStorePosFetchErrorMessage(e));
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  async function uploadGeneralStorePosImageFile(file: File): Promise<string> {
    const prepared = await prepareImageFileForUpload(file);
    const fd = new FormData();
    fd.append("file", prepared);
    const res = await fetch("/api/general-store-pos/upload", { method: "POST", body: fd, credentials: "include" });
  ...
  async function onCatImageFileChange(e: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      const url = await uploadGeneralStorePosImageFile(f);
      setCatImageUrl(url);
    } catch (err) {
  ...
  function openEditCategory(c: GeneralStorePosCategoryRow) {
    setCatEdit(c);
    setCatName(c.name);
  ...
  async function submitCategory() {
    ...
    try {
      ...
      if (catEdit) {
        const res = await fetch(`/api/general-store-pos/categories/${catEdit.id}`, {
          method: "PATCH",
        ...
        const res = await fetch("/api/general-store-pos/categories", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
          body: JSON.stringify(payload),
  ...
  async function deleteCategory(c: GeneralStorePosCategoryRow) {
    if (!window.confirm(`ลบหมวด «${c.name}»?`)) return;
    const res = await fetch(`/api/general-store-pos/categories/${c.id}`, { method: "DELETE", credentials: "include" });
    const j = await res.json().catch(() => ({}));
    if (!res.ok) {
  ...
  function openEditProduct(p: GeneralStorePosProductRow) {
    setProdEdit(p);
  ...
  async function submitProduct() {
    ...
    try {
      ...
      if (prodEdit) {
        const res = await fetch(`/api/general-store-pos/products/${prodEdit.id}`, {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
          body: JSON.stringify(payload),
        ...
        const res = await fetch("/api/general-store-pos/products", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
          body: JSON.stringify(payload),
  ...
  async function deleteProduct(p: GeneralStorePosProductRow) {
    if (!window.confirm(`ลบสินค้า «${p.name}»?`)) return;
    const res = await fetch(`/api/general-store-pos/products/${p.id}`, { method: "DELETE", credentials: "include" });
    const j = await res.json().catch(() => ({}));
    if (!res.ok) {
      window.alert(typeof j.error === "string" ? j.error : "ลบไม่สำเร็จ");
      return;
  ...
  function addProductToDraft(p: GeneralStorePosProductRow, delta: number) {
    if (delta <= 0) return;
    setDraftLines((prev) => {
  ...
  function handleProductCardTap(p: GeneralStorePosProductRow) {
    ...
    }, GENERAL_STORE_CARD_DOUBLE_TAP_MS);
  ...
  async function postSaleLines(lines: { productId: string; quantity: number }[]) {
    const res = await fetch("/api/general-store-pos/sales", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      credentials: "include",
      body: JSON.stringify({ note: null, lines }),
    });
    const j = await res.json().catch(() => ({}));
    if (!res.ok) {
        ...
        <AppSectionHeader
          ...
          action={
            <div className="flex shrink-0 flex-nowrap items-center gap-1.5 sm:gap-2">
              <Link
                href="/dashboard/general-store-pos/sales"
                aria-label="ดูยอดขาย"
                className={cn(
                  appTemplateOutlineButtonClass,
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <button
                ...
                onClick={() => {
                  setCatErr(null);
                  setCatImageUrl(suggestGeneralStorePosStockImageUrl({ name: catName, categoryName: catName }));
                }}
                className={cn(
                  appTemplateOutlineButtonClass,
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <button
                ...
                onClick={() => {
                  setProdErr(null);
                  setProdImageUrl(
                    suggestGeneralStorePosStockImageUrl({
                      name: prodName,
