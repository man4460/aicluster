...
export function GeneralStorePosShell({ children }: { children: React.ReactNode }) {
  ...
  const isProducts = onModule && !isSales;
  const [usageGuideOpen, setUsageGuideOpen] = useState(false);

  return (
    <GeneralStorePosMobileBottomProvider>
      <div className="flex min-h-0 max-w-full flex-1 flex-col gap-4 sm:gap-6">
        <header className={`${generalStorePosGlassShellClass} shrink-0 flex flex-col px-4 py-4 sm:px-8 sm:py-6 print:hidden`}>
          <div className="flex flex-wrap items-start justify-between gap-3 gap-y-2">
            <div className="flex min-w-0 items-start gap-3">
              ...
              <div className="min-w-0">
                <h1 className="text-xl font-black tracking-tight text-[#1e1b4b] sm:text-2xl">POS ร้านทั่วไป</h1>
              </div>
            </div>
            <button
              type="button"
          ...
          >
            <ul className="flex gap-1">
              ...
              <li className="min-w-0 flex-1">
                <TabLink href={`${base}/sales`} label="ยอดขาย" active={isSales} icon={<IconTabSales className="h-4 w-4" />} />
              </li>
            </ul>
          </nav>
        </header>
