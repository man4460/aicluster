...
export function VaultHeaderExpandButton({ onExpand }: { onExpand: () => void }) {
  return (
    <button
      type="button"
      onClick={onExpand}
...
export function VaultHeaderBarNav({ onExpand }: { onExpand: () => void }) {
    ...
    <div className={appDashboardModuleHeaderNavRowClass}>
      ...
      >
        {VAULT_NAV_ITEMS.map((item) => {
          const active = isVaultNavItemActive(pathname, item.key);
          return (
            <Link
              key={item.key}
              href={item.href}
      ...
      <VaultHeaderExpandButton onExpand={onExpand} />
    </div>
  );
}
