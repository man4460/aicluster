...
export type BarberPaymentPanelProps = {
  amountBaht: number;
  method: BarberPaymentMethod;
  slipUrl: string | null;
  onMethodChange: (method: BarberPaymentMethod) => void;
  onSlipUrlChange: (url: string | null) => void;
  disabled?: boolean;
  className?: string;
};

export function BarberPaymentPanel({
  amountBaht,
  method,
  slipUrl,
  onMethodChange,
  ...
  const needsSlip = barberPaymentShowsSlipUpload(method, amountBaht);
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const res = await fetch("/api/barber/promptpay-qr", {
          method: "POST",
          ...
          {BARBER_PAYMENT_METHODS.map((m) => (
            <button
              ...
              className={cn(
                ...
                method === m ? barberPaymentChipActiveClass : barberPaymentChipIdleClass,
              )}
              aria-pressed={method === m}
            >
              {barberPaymentMethodLabel(m)}
            </button>
          ))}
        </div>
      </div>
        ...
        <div className="space-y-2 rounded-[1.25rem] border border-[#e8e6fc]/90 bg-white/80 p-3">
          ...
          {info?.qrDataUrl ? (
            <button
              type="button"
              disabled={disabled}
              className={cn(barberPaymentCtaClass, "w-full min-h-[44px] rounded-2xl text-sm")}
        ...
        ? createPortal(
            <div className="fixed inset-0 z-[230] flex items-center justify-center p-4 sm:p-6" role="presentation">
              ...
              <div
                role="dialog"
                aria-modal="true"
                aria-labelledby="barber-customer-pp-qr-title"
              ...
              >
                <p
                  id="barber-customer-pp-qr-title"
      ...
      {showTransfer ? (
        <div className="space-y-1.5 rounded-[1.25rem] border border-[#e8e6fc]/90 bg-white/80 p-3 text-xs font-semibold text-[#66638c]">
          ...
          {qrBusy && !info ? (
            ...
            <p className="font-bold text-rose-600">{qrErr || "ยังไม่ได้ตั้งบัญชีโอน — ไปที่ตั้งค่าร้าน"}</p>
          )}
        </div>
      ) : null}
