...
export function BarberPortalMediaSettings({
  bannerUrl,
  gallery,
  facebookUrl,
  mapUrl,
  ...
  async function uploadFile(file: File): Promise<string> {
    ...
    const res = await fetch("/api/barber/packages/upload", {
      method: "POST",
      body: fd,
      credentials: "include",
    });
  ...
  async function onPickGallery(e: ChangeEvent<HTMLInputElement>) {
    ...
    const slots = BARBER_PORTAL_GALLERY_MAX - gallery.length;
    if (slots <= 0) {
      setErr(`อัปโหลดได้ไม่เกิน ${BARBER_PORTAL_GALLERY_MAX} รูป`);
      return;
    }
    setUploadBusy(true);
    setErr(null);
    try {
      ...
      onGalleryChange([...gallery, ...added].slice(0, BARBER_PORTAL_GALLERY_MAX));
    } catch (e2) {
    ...
    <div className="space-y-5 rounded-2xl border border-[#ecebff] bg-[#faf9ff]/80 p-3 sm:p-4">
      ...
      <div>
        ...
        <div className="mt-2 flex flex-wrap gap-2">
          ...
          {!bannerUrl ? (
            <button
              type="button"
              disabled={busy}
              onClick={() => onBannerUrlChange(BARBER_PORTAL_SAMPLE_BANNER)}
        ...
        {bannerUrl ? (
          ...
          >
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img
              src={BARBER_PORTAL_SAMPLE_BANNER}
      ...
      <div>
        <p className="text-xs font-bold text-[#4d47b6]">
          แกลเลอรี ({gallery.length}/{BARBER_PORTAL_GALLERY_MAX})
        </p>
        <input
          ref={galleryPickRef}
          type="file"
        ...
        <div className="mt-2 flex flex-wrap gap-2">
          <AppImagePickCameraButtons
            ...
            onPickCamera={() =>
              galleryCamera.openCamera(async (file) => {
                if (gallery.length >= BARBER_PORTAL_GALLERY_MAX) {
                  setErr(`อัปโหลดได้ไม่เกิน ${BARBER_PORTAL_GALLERY_MAX} รูป`);
                  return;
                }
                setUploadBusy(true);
                ...
                try {
                  ...
                  onGalleryChange([...gallery, url].slice(0, BARBER_PORTAL_GALLERY_MAX));
                } catch (e2) {
                  setErr(e2 instanceof Error ? e2.message : "อัปโหลดไม่สำเร็จ");
                } finally {
                  setUploadBusy(false);
            ...
            disabled={busy || gallery.length >= BARBER_PORTAL_GALLERY_MAX}
            busy={uploadBusy}
          ...
          <button
            type="button"
            disabled={busy}
            onClick={() => onGalleryChange([...BARBER_PORTAL_SAMPLE_GALLERY])}
            className={cn(appTemplateOutlineButtonClass, "min-h-[44px] rounded-xl px-3 text-xs font-bold")}
          >
        ...
        {gallery.length ? (
          ...
          <ul className="mt-3 grid grid-cols-3 gap-2 sm:grid-cols-4">
            {BARBER_PORTAL_SAMPLE_GALLERY.map((url, idx) => (
              <li key={`sample-${idx}`}>
                <button
                  type="button"
                  onClick={() => onGalleryChange([...BARBER_PORTAL_SAMPLE_GALLERY])}
