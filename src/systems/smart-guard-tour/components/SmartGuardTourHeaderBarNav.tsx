...
function SmartGuardTourHeaderExpandGlyph() {
  return (
...
export function SmartGuardTourHeaderExpandButton({ onExpand }: { onExpand: () => void }) {
  return (
    <button
      type="button"
      onClick={onExpand}
    ...
    >
      <SmartGuardTourHeaderExpandGlyph />
    </button>
  );
}

function SmartGuardTourHeaderBarNavInner({ onExpand }: { onExpand: () => void }) {
  const pathname = (usePathname() ?? "").replace(/\/+$/, "");

  return (
    <div className={appDashboardModuleHeaderNavRowClass}>
      <nav className={appDashboardModuleHeaderNavShellClass} aria-label="เมนูโมดูลจุดตรวจ รปภ.">
      ...
      <span className={appDashboardModuleHeaderTitleClass}>{SMART_GUARD_TOUR_MODULE_DISPLAY_NAME}</span>
      <SmartGuardTourHeaderExpandButton onExpand={onExpand} />
    </div>
  );
}

/** แถบเมนูหลักใน header ม่วงเมื่อย่อหัวโมดูล */
export function SmartGuardTourHeaderBarNav({ onExpand }: { onExpand: () => void }) {
  return (
    <Suspense fallback={null}>
      <SmartGuardTourHeaderBarNavInner onExpand={onExpand} />
    </Suspense>
  );
}
