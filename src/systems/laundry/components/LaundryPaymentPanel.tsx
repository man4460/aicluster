...
import { uploadLaundrySessionImage } from "@/systems/laundry/laundry-service";
...
export type LaundryPaymentPanelProps = {
  amountBaht: number;
  method: LaundryPaymentMethod;
  slipUrl: string | null;
  onMethodChange: (method: LaundryPaymentMethod) => void;
...
export function LaundryPaymentPanel({
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const res = await fetch("/api/laundry/promptpay-qr", {
          method: "POST",
  ...
  return (
    <div
      className={cn(
        "space-y-3 border-t border-slate-200/80 pt-3",
        className,
      )}
    >
      <div>
...
export { isLaundryPaymentMethod, laundryPaymentMethodLabel };
