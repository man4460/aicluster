...
export type LaundryMemberPrintRow = {
  id: number;
  createdAt: string;
...
type Props = {
  open: boolean;
  row: LaundryMemberPrintRow | null;
...
export function LaundryMemberPrintModal({
  open,
  row,
  shop,
  ...
  useEffect(() => {
    ...
    setBillingName((row.customer.billingName || row.customer.name || "").trim());
          ...
          <div className="rounded-2xl border border-[#ecebff] bg-[#faf9ff]/90 px-3 py-3">
            ...
            <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs font-semibold">
              <span className="text-[#66638c]">
                ยอด <span className="font-black text-[#1e1b4b]">{formatLaundryPrintBaht(priceBaht)}</span>
              </span>
              <span className="text-[#66638c]">
                ช่องทาง{" "}
                <span className="font-black text-[#1e1b4b]">{laundryPaymentMethodLabel(row.paymentMethod)}</span>
              </span>
            </div>
          </div>
