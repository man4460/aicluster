...
function RevenueSlipAttachmentZone({
  slipUrl,
  onSlipUrlChange,
  photoBusy,
  previewUrl,
...
export type LaundryRevenuePanelHandle = {
  openAddEntry: () => void;
  openManageCategories: () => void;
  openEditEntry: (e: LaundryRevenueEntry) => void;
};

export const LaundryRevenuePanel = forwardRef<
  LaundryRevenuePanelHandle,
  {
...
>(function LaundryRevenuePanel(
  { repo, baseUrl, categories, entries, onRefresh, modalsOnly = false },
  ref,
) {
  ...
  const [editCat, setEditCat] = useState<LaundryRevenueCategory | null>(null);

  const [showAddEntryModal, setShowAddEntryModal] = useState(false);
  const [entryCategoryId, setEntryCategoryId] = useState("");
  const [entryEarnedLocal, setEntryEarnedLocal] = useState(() => isoToDatetimeLocalInput(new Date().toISOString()));
  const [entryAmount, setEntryAmount] = useState("");
  const [entryItemLabel, setEntryItemLabel] = useState("");
  const [entryNote, setEntryNote] = useState("");
  const [entrySlipUrl, setEntrySlipUrl] = useState("");

  const [editEntry, setEditEntry] = useState<LaundryRevenueEntry | null>(null);
  const [editEntryForm, setEditEntryForm] = useState<{
    category_id: string;
    earned_at_local: string;
    amount: string;
    item_label: string;
    note: string;
  ...
  const sortedEntries = useMemo(
    () => [...entries].sort((a, b) => new Date(b.earned_at).getTime() - new Date(a.earned_at).getTime()),
    [entries],
  );
  ...
  const openEditCategoryForm = useCallback((c: LaundryRevenueCategory) => {
    setEditCat(c);
    setCatName(c.name);
  }, []);
  ...
  const submitCategory = useCallback(async () => {
    ...
    setErr(null);
    try {
      if (editCat) await repo.updateRevenueCategory(editCat.id, { name: n });
      else await repo.createRevenueCategory(n);
      cancelCategoryForm();
      await onRefresh();
    } catch (e) {
  ...
  const removeCategory = useCallback(
    async (c: LaundryRevenueCategory) => {
      if (!confirm(`ลบหมวด «${c.name}» และรายการรายรับที่ผูกไว้ทั้งหมด?`)) return;
      setBusy(true);
      setErr(null);
      try {
        await repo.deleteRevenueCategory(c.id);
        await onRefresh();
      } catch (e) {
  ...
  const resetAddEntryForm = useCallback(() => {
    setEntryCategoryId("");
    setEntryEarnedLocal(isoToDatetimeLocalInput(new Date().toISOString()));
    setEntryAmount("");
    setEntryItemLabel("");
    setEntryNote("");
    setEntrySlipUrl("");
  ...
  const finalizeSlipFile = useCallback(async (file: File, target: "add" | "edit") => {
    ...
    try {
      ...
      const url = await uploadLaundrySessionImage(prepared);
  ...
  const submitEntry = useCallback(async () => {
    ...
    try {
      await repo.createRevenueEntry({
        category_id: cid,
        earned_at: datetimeLocalToIso(entryEarnedLocal),
  ...
  }, [entryAmount, entryCategoryId, entryItemLabel, entryNote, entrySlipUrl, entryEarnedLocal, onRefresh, repo, resetAddEntryForm]);

  const openEditEntry = useCallback((e: LaundryRevenueEntry) => {
    setEditEntry(e);
    setEditEntryForm({
      category_id: String(e.category_id),
      earned_at_local: isoToDatetimeLocalInput(e.earned_at),
  ...
  useImperativeHandle(
    ...
    [openAddEntry, openManageCategories, openEditEntry],
  );
  ...
  const submitEditEntry = useCallback(async () => {
    ...
    setErr(null);
    try {
      await repo.updateRevenueEntry(editEntry.id, {
        category_id: cid,
        earned_at: datetimeLocalToIso(editEntryForm.earned_at_local),
        amount: Math.round(amt),
        item_label: item,
  ...
  const removeEntry = useCallback(
    async (e: LaundryRevenueEntry) => {
      if (!confirm("ลบรายการรายรับนี้?")) return;
      setBusy(true);
      setErr(null);
      try {
        await repo.deleteRevenueEntry(e.id);
        await onRefresh();
      } catch (er) {
  ...
  return (
    <div className={modalsOnly ? "contents" : "space-y-6"}>
        ...
        <>
          ...
          <div className="flex items-start justify-between gap-3 sm:items-center sm:gap-4">
            <div className="min-w-0 text-xs text-slate-500">
              <p className="text-sm font-semibold text-slate-900">รายรับเพิ่ม</p>
              <p className="mt-0.5 text-xs font-semibold tabular-nums text-emerald-700">
                รวมทั้งสิ้น ฿{totalRevenueAmount.toLocaleString("en-US")}
              </p>
              {categories.length === 0 ?
          ...
          <div>
            {sortedEntries.length === 0 ?
              <AppEmptyState>ยังไม่มีรายการรายรับ</AppEmptyState>
                        ...
                        <div className="flex min-w-0 items-start gap-3">
                          ...
                          <div className="min-w-0 flex-1">
                            <p className="text-xs tabular-nums text-slate-500">
                              {new Date(e.earned_at).toLocaleString("th-TH", { timeZone: "Asia/Bangkok" })}
                            </p>
                            <p className="font-semibold text-slate-900">
                              {e.item_label?.trim() || "—"}
                              <span className="ml-1.5 font-normal text-slate-500">({e.category_name})</span>
                            </p>
                            <p className="text-lg font-bold tabular-nums text-emerald-700">฿{e.amount.toLocaleString()}</p>
      ...
      <FormModal
        ...
        title="หมวดรายรับ"
        description="สร้างหรือลบหมวดหมู่สำหรับรายรับ"
        size="md"
        mobileCentered
        footer={
      ...
      <FormModal
        ...
        title="บันทึกรายการรายรับ"
        description="กรอกข้อมูลรายรับและเลือกหมวดหมู่ — รูปภาพสลิปจะถูกอัปโหลดทันที"
        size="md"
        mobileCentered
        footer={
          <FormModalFooterActions
        ...
        <div className="space-y-6">
          ...
          {categories.length === 0 ?
            <div className="rounded-xl border border-amber-200 bg-amber-50 px-3 py-2.5 text-sm text-amber-900">
              <p className="font-semibold">ยังไม่มีหมวดรายรับ</p>
              <button
                type="button"
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5">
              ...
              <input
                type="datetime-local"
                className="w-full rounded-xl border-slate-200 bg-slate-50/50 text-sm font-semibold focus:ring-[#5b61ff]"
                value={entryEarnedLocal}
                onChange={(ev) => setEntryEarnedLocal(ev.target.value)}
              />
            </div>
          </div>
          ...
          <div className="space-y-4 rounded-2xl border border-slate-100 bg-slate-50/30 p-4">
            <div className="space-y-1.5">
              ...
              <input
                ...
                placeholder="เช่น รายได้อื่น ๆ, ค่าบริการเสริม…"
            ...
            <div className="space-y-1.5">
              ...
              <input
                type="text"
                inputMode="decimal"
                className="w-full rounded-xl border-slate-200 bg-white px-3 py-2.5 text-lg font-black tabular-nums text-emerald-700 focus:ring-emerald-500"
                placeholder="0"
                value={entryAmount}
      ...
      <FormModal
        ...
        title={editEntry ? `แก้ไขรายการรายรับ #${editEntry.id}` : "แก้ไข"}
        description="อัปเดตข้อมูลรายรับ"
        size="md"
        mobileCentered
        footer={
          editEntryForm ? (
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5">
                ...
                <input
                  type="datetime-local"
                  className="w-full rounded-xl border-slate-200 bg-slate-50/50 text-sm font-semibold focus:ring-[#5b61ff]"
                  value={editEntryForm.earned_at_local}
                  onChange={(ev) => setEditEntryForm((s) => (s ? { ...s, earned_at_local: ev.target.value } : s))}
                />
              </div>
            </div>
            ...
            <div className="space-y-4 rounded-2xl border border-slate-100 bg-slate-50/30 p-4">
              ...
              <div className="space-y-1.5">
                ...
                <input
                  type="text"
                  inputMode="decimal"
                  className="w-full rounded-xl border-slate-200 bg-white px-3 py-2.5 text-lg font-black tabular-nums text-emerald-600 focus:ring-emerald-500"
                  placeholder="0"
...
LaundryRevenuePanel.displayName = "LaundryRevenuePanel";
