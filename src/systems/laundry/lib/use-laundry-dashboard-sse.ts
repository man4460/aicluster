...
const STREAM_URL = "/api/laundry/session/dashboard/stream";

/**
 * SSE แดชบอร์ดซักผ้า + soft-poll เมื่อต่อ SSE ได้ / poll ถี่เมื่อหลุด
 * เรียก `onRefresh` เมื่อมี event หรือตามช่วงเวลาสำรอง
 */
export function useLaundryDashboardSse(onRefresh: () => void, enabled = true): void {
