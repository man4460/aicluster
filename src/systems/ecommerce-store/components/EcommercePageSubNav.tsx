...
import {
  ...
  ecommerceStorePrimaryTabPillClass,
  ecommerceStorePrimaryTabShellClass,
...
const tabBtnClass = (active: boolean) =>
  cn(
    ecommerceStorePrimaryTabPillClass(active),
    "inline-flex min-h-9 flex-1 items-center justify-center gap-1.5 sm:flex-initial",
  );

/** แถบเมนูย่อยของแต่ละหน้าหลัก — แยกจากเนื้อหา */
export function EcommercePageSubNav({
  ...
  return (
    <div className={cn(ecommerceStorePanelClass, ecommerceStorePanelSectionClass, "print:hidden", className)}>
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
        <div className="min-w-0 flex-1">
          ...
          {description ? <p className={ecommerceStoreSubtitleClass}>{description}</p> : null}
        </div>
