...
function EcommerceManageHubTabs() {
  ...
  // URL เก่า `?tab=stock` → รวมเข้าสินค้าแล้ว
  useEffect(() => {
    if (searchParams.get("tab") === "stock") {
      router.replace(ecommerceStoreManageHref("products"), { scroll: false });
    }
  }, [router, searchParams]);
  ...
  const productsActions = (
    <div className={ecommerceStoreInlineSubNavShellClass}>
      ...
      <button
        ...
        disabled={!productsToolbar}
        onClick={() => productsToolbar?.downloadTemplate()}
      >
      ...
      <button
        ...
        disabled={!productsToolbar || productsToolbar.importBusy}
        onClick={() => productsToolbar?.triggerImport()}
      >
      ...
      <button
        ...
        disabled={!productsToolbar}
        onClick={() => productsToolbar?.downloadExport()}
      >
        <Download className="h-3.5 w-3.5 shrink-0" aria-hidden />
      ...
      <button
        ...
        disabled={!productsToolbar || productsToolbar.loading}
        onClick={() => productsToolbar?.reload()}
      >
        <RefreshCw className={cn("h-3.5 w-3.5 shrink-0", productsToolbar?.loading && "animate-spin")} aria-hidden />
      </button>
      <button
        type="button"
        className={ecommerceStoreInlineSubNavBtnClass(false)}
