...
/**
 * Pub/sub ในโปรเซสเดียวกันสำหรับแดชบอร์ดร้านออนไลน์ (SSE)
 * single Node / next start — หลาย instance ต้องใช้ Redis ภายหลัง
 */
const listenersByOwner = new Map<string, Set<Listener>>();

export function subscribeEcommerceDashboard(ownerUserId: string, listener: Listener): () => void {
...
export function publishEcommerceDashboard(ownerUserId: string): void {
